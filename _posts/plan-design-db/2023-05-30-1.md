...
```
현재 테이블에 생성된 모든 인덱스를 확인할 수 있습니다.
<table class="table_2_left">
	<tbody>
	<tr>
		<td>Key_name</td>
		<td>PK 인덱스는 PRIMARY, 논클러스터드 인덱스는 인덱스명</td>
	</tr>
	<tr>
		<td>Seq_in_index</td>
		<td>인덱스에 현재 컬럼이 포함된 순서</td>
	</tr>
	<tr>
		<td>Column_name</td>
		<td>인덱스가 적용된 컬럼명</td>
	...
	<tr>
		...
		<td>
			고유 값의 수<br>
			카디널리티가 높은 = 유니크한 값이 많은 컬럼에 인덱스를 걸면 쿼리 성능이 더 좋습니다.
		</td>
	</tr>
	</tbody>
</table>
...
날짜+시간처럼 카디널리티가 높은 하나의 컬럼에는 단일 컬럼 인덱스를 생성해도 좋습니다.

<mark>인덱스 장단점</mark>  
인덱스를 생성하면 조회 성능은 좋아지지만 INSERT, UPDATE, DELETE 성능이 저하됩니다.  
특히, 데이터가 많아질수록 삽입 속도가 점점 느려지는 것을 확인할 수 있습니다.  
B-Tree 자료구조로 정렬된 인덱스 데이터도 함께 관리하기 때문입니다.

한 테이블에 4~5개 정도의 인덱스 수가 권장됩니다.

단일 컬럼 인덱스 설정 시, 멀티 컬럼 인덱스 설정 시 성능 차이가 크지 않으면  
단일 컬럼 인덱스로 인덱스를 최소화 하는 것이 효율적일 수 있습니다.

<mark>인덱스 생성하면 좋은 컬럼 ★</mark>
- 테이블 수정 (INSERT, UPDATE, DELETE) 보다 테이블 조회 (SELECT) 작업이 많은 컬럼
- Join 조건 또는 WHERE 절에서 자주 사용되는 컬럼
- = (동등 조건) 으로 조회되는 컬럼
- IN 조건에 사용되는 컬럼
- 부등호(>, <, ≤, ≥), BETWEEN, LIKE '값%' 등 범위 조건에 사용되는 컬럼
- 선택도가 10% 이하로 낮은 컬럼 (데이터가 많이 걸러지는 컬럼)
- 카디널리티가 높은 컬럼 (고유 값의 수가 많아 중복이 적은 컬럼)
...
```
이중화 된 네이버 클라이드 MySQL DB 사용 시에는 모든 테이블에 PK 추가가 권장됩니다.  
ROW 기반 복제나 InnoDB 클러스터에서는 모든 테이블이 PK를 가져야 정상적인 복제 성능을 보장하므로,  
PK가 없는 데이블이 존재하면 행 식별 과정에서 데이터 정합성이 깨져 Standby DB 복제가 중단될 수 있습니다.
