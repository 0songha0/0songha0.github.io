...
permalink: /plan-design-db/2025-11-03-1
...
싱글턴 패턴은 주로 생성자를 private으로 만들고,  
static 메소드에서 유일한 인스턴스를 생성 및 반환하는 방식으로 구현됩니다.
...
복잡한 객체의 생성 과정과 구성 방법을 단계별로 분리하고 캡슐화하여  
다양한 구성의 객체를 유연하게 생성할 수 있는 패턴입니다.

선택적 매개변수가 많은 객체를 생성할 때 코드의 가독성과 유지보수성을 크게 향상시킬 수 있습니다.
...
class Pizza {
  ...
  private Pizza(PizzaBuilder builder) {
    // PizzaBuilder 인스턴스 정보로 Pizza 생성
    this.dough = builder.dough;
    this.sauce = builder.sauce;
    this.topping = builder.topping;
  }

  // Pizza 인스턴스 출력 시 문자열 커스텀
  @Override
  ...
  public static class PizzaBuilder {
    ...
    // 생성자를 직접 선언하지 않았으므로,
    // Java 컴파일러가 public 기본 생성자 자동 제공

    // 현재 PizzaBuilder 인스턴스에 도우 값 할당
    public PizzaBuilder dough(String dough) {
      this.dough = dough;

      // PizzaBuilder 인스턴스 반환 ★
      // 호출부에서 메서드 체이닝 사용 가능
      return this;
    }

    // 현재 PizzaBuilder 인스턴스에 소스 값 할당
    ...
    // 제품 인스턴스 생성 및 반환 함수 ★
    public Pizza build() {
      // PizzaBuilder 인스턴스 정보로 Pizza 인스턴스 생성
...
public class Main {
  public static void main(String[] args) {
    // 예시 1 : 생성할 객체 정보가 준비된 경우

    // Pizza 클래스 내부 PizzaBuilder 인스턴스 생성 후
    // 메서드 체이닝 (메소드 연속 호출)
    ...
    // PizzaBuilder를 통해 생성된 Pizza 인스턴스 출력
    System.out.println(myPizza);

    // 예시 2 : 생성할 객체 정보가 실행 중에 결정되는 경우

    // PizzaBuilder 생성 후 도우 정보 할당
    Pizza.PizzaBuilder pizzaBuilder = new Pizza.PizzaBuilder().dough("Regular");

    // PizzaBuilder에 소스 정보 할당
    ...
    // PizzaBuilder를 통해 Pizza 인스턴스 생성
...
public class HttpRequest {
  ...
  public static class Builder {
    ...
    // Builder 생성 시 제품 필수 데이터 할당
    public Builder(String method, String url) {
      this.method = method;
      this.url = url;
    }

    // 현재 Builder 인스턴스에 헤더 값 추가
    ...
    public HttpRequest build() {
      // Builder 인스턴스 정보로 HttpRequest 인스턴스 생성
...
복잡한 설정을 가진 객체처럼 생성 및 초기화 비용이 크거나,   
동일하거나 유사한 객체를 많이 만들어야 할 때  
기존 객체를 복제함으로써 객체 생성 비용을 줄일 수 있습니다.

일반적으로 프로토타입 객체가 자기 복제 기능을 제공하도록 구현합니다.

<mark>프로토타입 패턴 예시 1</mark>
```
// 프로토타입 인터페이스
// Java 기본 라이브러리의 Cloneable 인터페이스를 활용하여 구현할 수도 있음
interface Prototype {
    // 복제된 인스턴스 반환 함수 정의
    Prototype clone();
}

// 프로토타입 인터페이스를 구현한 사람 클래스
class Person implements Prototype {
  private String name;
  private int age;
  private String address;

  // 각 필드 값을 매개변수로 받아 할당하는 생성자
  public Person(
  ...
  // 같은 클래스 인스턴스를 매개변수로 받아 필드 값을 복사하는 생성자
...
// 프로토타입 인터페이스를 상속받은 문서 인터페이스
interface Document extends Prototype {
  // 복제된 문서 인스턴스 반환 함수 정의
  @Override
  Document clone();
...
### 복합체 패턴 (Composite)
객체 컴포넌트들을 트리 구조로 구성하여 부분-전체 계층 구조를 표현할 수 있는 패턴입니다.  
클라이언트는 복합 객체와 개별 객체를 구분하지 않고 동일하게 다룰 수 있습니다.

폴더처럼 다른 요소들을 하위로 포함할 수 있는 요소들은 복합체이고,  
파일처럼 하위 요소를 갖지 못하는 요소들은 잎 (leaf) 입니다.
...
// 파일 시스템 컴포넌트 인터페이스를 구현한 Leaf 클래스 : 파일
class File implements FileSystemComponent {
  // 파일 이름, 사이즈 필드
  private String name;
  private int size;

  // 생성자에서 매개변수로 받은 파일 이름, 사이즈 저장
...
// 파일 시스템 컴포넌트 인터페이스를 구현한 복합체 클래스 : 폴더
class Directory implements FileSystemComponent {
  // 폴더 이름 필드
  private String name;

  // 하위 파일 시스템 컴포넌트 리스트 필드
  ...
  // 다른 컴포넌트를 받아 리스트에 추가하는 함수
  public void add(FileSystemComponent component) {
    components.add(component);
  }

  // 컴포넌트를 참조받아 리스트에서 삭제하는 함수
  public void remove(FileSystemComponent component) {
    components.remove(component);
  }

  // 이름에 해당하는 컴포넌트를 리스트에서 삭제하는 함수
  ...
  public void printName() {
    ...
    // 폴더 내 모든 컴포넌트 요소 이름 출력
    for (FileSystemComponent component : components) {
      // 컴포넌트 요소가 파일이면 이름을 출력하고,
      // 폴더면 재귀적으로 모든 하위 폴더 및 파일 이름 출력
      component.printName();
    }
  }
  ...
  public int getSize() {
    ...
    // 폴더 내 모든 컴포넌트 사이즈 구하기
    for (FileSystemComponent component : components) {
      // 컴포넌트 요소가 파일이면 사이즈를 더하고,
      // 폴더면 재귀적으로 모든 하위 폴더 및 파일 사이즈 누적
...
public class Main {
  public static void main(String[] args) {
    ...
    // 최상위 폴더에서 이름 출력 함수 호출 시 내부 모든 폴더 및 파일 이름 출력
    rootDir.printName();
    // 최상위 폴더에서 사이즈 반환 함수 호출 시 내부 모든 폴더 및 파일 사이즈 누적
    System.out.println("Total size: " + rootDir.getSize() + " KB");

    // Image.jpg 이름을 가진 컴포넌트 삭제 후 재출력
    System.out.println("\nRemoving Image.jpg:");
    rootDir.remove("Image.jpg");
    rootDir.printName();
    System.out.println("Total " + rootDir.getSize() + " KB");

    // subDir 컴포넌트 삭제 후 재출력
    System.out.println("\nRemoving SubFolder:");
...
// UI 컴포넌트 인터페이스를 구현한 Leaf 클래스 1 : 버튼
class Button implements UIComponent {
  ...
  public void add(UIComponent component) {
    // 잎이니까 다른 컴포넌트 추가 시 예외 반환 (인터페이스 분리 원칙 위배)
  ...
  public void remove(UIComponent component) {
    // 잎이니까 다른 컴포넌트 삭제 시 예외 반환 (인터페이스 분리 원칙 위배)
    throw new UnsupportedOperationException();
  }
}

// UI 컴포넌트 인터페이스를 구현한 Leaf 클래스 2 : 텍스트 박스
class TextBox implements UIComponent {
  ...
  @Override
  public void remove(UIComponent component) {
    // 잎이니까 다른 컴포넌트 삭제 시 예외 반환
    throw new UnsupportedOperationException();
  }
}

// UI 컴포넌트 인터페이스를 구현한 복합체 클래스 1 : 패널
class Panel implements UIComponent {
  ...
  // 컴포넌트 정보 출력 함수 구현 (재귀적으로 전체 하위 트리 렌더링)
  @Override
  public void render() {
    // 현재 컴포넌트 정보 출력
    System.out.println("Panel: " + name);

    // 하위 컴포넌트들 정보 출력 함수 호출
...
// UI 컴포넌트 인터페이스를 구현한 복합체 클래스 2 : 윈도우
...
public class Main {
  public static void main(String[] args) {
    ...
    // 패널 컴포넌트 인스턴스 생성 후 Leaf 컴포넌트 요소들 추가
    Panel formPanel = new Panel("Form");
    formPanel.add(submitButton);
    formPanel.add(cancelButton);
    formPanel.add(usernameField);

    // 윈도우 컴포넌트 인스턴스 생성 후 패널 컴포넌트 인스턴스 추가
    Window mainWindow = new Window("Main");
    mainWindow.add(formPanel);

    // 윈도우 컴포넌트 정보 출력 함수 호출
    mainWindow.render();

    System.out.println();

    // 패널에서 취소 버튼 삭제 후 윈도우 컴포넌트 정보 출력 함수 호출
    formPanel.remove(cancelButton);
    mainWindow.render();
  }
}
```
Composite 패턴의 투명성 중시 방식 예시입니다.  
Leaf 클래스는 add 및 remove 기능이 필요하지 않지만, 공통 인터페이스에 정의되어 구현을 강제받습니다.  
Leaf가 사용하지 않는 메서드에 의존하게 되므로 인터페이스 분리 원칙에는 위배될 수 있습니다.  
위 코드의 실행 결과는 아래와 같습니다.
```
Window: Main
Panel: Form
...
기존 객체 클래스를 수정하지 않고, 다른 객체로 감싸 새로운 책임 및 기능을 동적으로 추가하고 조합할 수 있는 패턴입니다.
...
public class Main {
  public static void main(String[] args) {
    ...
    // 데코레이션 대상 객체에 우유 데코레이터 기능 추가
    Coffee milkCoffee = new MilkDecorator(new SimpleCoffee());

    System.out.println(milkCoffee.getDescription() + " $" + milkCoffee.getCost());

    // 데코레이션 대상 객체에 우유 및 설탕 데코레이터 기능 조합
...
<mark>프록시 패턴 사용 예시</mark>
- 무거운 작업이 포함된 실제 객체 생성을 필요한 시점까지 지연
- 생성한 객체 또는 작업 결과를 캐싱
- 접근 제어가 필요한 로직을 분리 관리

<mark>프록시 패턴 예시 1 : 대용량 이미지 파일</mark>
```
// 주제 인터페이스
interface Image {
  void display();
...
주어진 요청을 여러 핸들러 객체의 체인을 따라 전달하며 처리할 기회를 주는 패턴입니다.  
각 핸들러 객체는 자기가 요청을 처리할 수 없으면 다음 핸들러에게 넘깁니다.  
같은 유형의 다른 객체를 후임자로 연결하여 다양한 조합의 작업 체인을 만들 수 있습니다.

요청을 보내는 객체는 실제로 어떤 핸들러가 요청을 처리하는지 알 필요가 없기 때문에,  
객체 간의 결합도를 낮추고 요청 처리의 유연성과 확장성을 높일 수 있습니다.

<mark>책임 연쇄 패턴 예시 1</mark>
```
// 핸들러 추상 클래스
abstract class Handler {
    // 현재 인스턴스 뒤에 연결될 다음 핸들러 필드
    ...
    public void handle(int number) {
        // 현재 핸들러가 작업 수행 후, 처리하지 못한 경우에만 다음 핸들러 핸들 함수 호출
        if (!process(number) && next != null) {
            next.handle(number);
        }
    }

    // 주어진 요청에 대한 작업 수행 후 처리결과 반환 함수
    protected abstract boolean process(int number);
}

// 핸들러 추상 클래스를 구현한 핸들러 클래스 1
class PositiveHandler extends Handler {
    @Override
    protected boolean process(int number) {
        // 매개변수로 받은 숫자가 0보다 큰 양수인 경우 텍스트 출력
        if (number > 0) {
...
// 핸들러 추상 클래스를 구현한 핸들러 클래스 2
class EvenHandler extends Handler {
    @Override
    protected boolean process(int number) {
        // 매개변수로 받은 숫자가 짝수인 경우 텍스트 출력
        if (number % 2 == 0) {
...
// 핸들러 추상 클래스를 구현한 핸들러 클래스 3
class DivisibleBy3Handler extends Handler {
    @Override
    protected boolean process(int number) {
        // 매개변수로 받은 숫자가 3의 배수인 경우 텍스트 출력
        if (number % 3 == 0) {
...
public class Main {
    public static void main(String[] args) {
        ...
        // 첫번째 핸들러 클래스에 두번째 핸들러 저장
        positive.setNext(even);

        // 두번째 핸들러 클래스에 세번째 핸들러 저장
        even.setNext(divisibleBy3);

        // 첫번째 핸들러 클래스 핸들 함수 호출
        // 현재 핸들러가 처리하지 못한 경우, 요청이 처리될 때까지 연결된 다음 핸들러 함수들도 순차적으로 호출 됨
        positive.handle(-2);
        positive.handle(4);
        positive.handle(6);
    }
...
// 열거형 로그 레벨
enum LogLevel {
  INFO, DEBUG, WARN // 선언 순서에 따라 ordinal() 호출 시 INFO=0, DEBUG=1, WARN=2 반환
}
...
abstract class Logger {
  ...
  public void logMessage(LogLevel level, String message) {
    // 현재 인스턴스 로그레벨 값이 주어진 레벨 값과 같거나 작을 때
    if (this.level.ordinal() <= level.ordinal()) {
      // 현재 로거 인스턴스 로그 메시지 출력 함수 호출
      write(message);
    }

    // 연결된 다음 로거가 있으면,
    if (nextLogger != null) {
      // 다음 로거 인스턴스 로그 메시지 출력 함수 호출
...
// 로거 추상 클래스를 상속받은 구상 클래스 1 : 콘솔 로거
class ConsoleLogger extends Logger {
  // 생성자에서 로그 레벨 값을 받아 저장
...
// 로거 추상 클래스를 상속받은 구상 클래스 2 : 파일 로거
...
// 로거 추상 클래스를 상속받은 구상 클래스 3 : 네트워크 로거
...
public class Main {
  ...
  private static Logger getChainOfLoggers() {
    // 로거 구상 클래스 인스턴스들 생성하며 각 로그 레벨 세팅
    Logger networkLogger = new NetworkLogger(LogLevel.WARN); // 네트워크 로거는 경고 레벨 세팅
    Logger fileLogger = new FileLogger(LogLevel.DEBUG); // 파일 로거는 디버그 레벨 세팅
    Logger consoleLogger = new ConsoleLogger(LogLevel.INFO); // 콘솔 로거는 정보 레벨 세팅

    // 네트워크 로거 다음에 파일 로거 저장
    networkLogger.setNextLogger(fileLogger);

    // 파일 로거 다음에 콘솔 로거 저장
    fileLogger.setNextLogger(consoleLogger);

    // 네트워크 로거 > 파일 로거 > 콘솔 로거가 연결된 네트워크 로거 반환
    return networkLogger;
  }

  public static void main(String[] args) {
    // 로거 체인 저장 (다음 로거들이 연결된 첫 로거 저장)
    Logger loggerChain = getChainOfLoggers();

    // 로거 메시지 함수 호출 시 정보 레벨 입력하면 콘솔 로거에서만 메시지 출력 됨
    loggerChain.logMessage(LogLevel.INFO, "This is an information.");
    
    // 로거 메시지 함수 호출 시 디버그 레벨 입력하면 파일 로거, 콘솔 로거에서만 메시지 출력 됨
    loggerChain.logMessage(LogLevel.DEBUG, "This is a debug level information.");
    
    // 로거 메시지 함수 호출 시 경고 레벨 입력하면 네트워크 로거, 파일 로거, 콘솔 로거에서 메시지 출력 됨
    loggerChain.logMessage(LogLevel.WARN, "This is a warning information.");
  }
}
```
요청이 끝까지 전달되면서 조건을 만족하는 여러 핸들러가 모두 처리하는 책임 연쇄 변형 예시입니다.  
위 코드의 실행 결과는 아래와 같습니다.
```
...
### 커맨드 패턴 (Command)
명령을 객체로 캡슐화하여, 명령을 실행시키는 인보커 객체와  
작업을 수행하는 리시버 객체를 분리하는 패턴입니다.

명령 객체를 조합하여 원하는 시점에 실행할 수 있습니다.  
실행 취소, 재실행, 로깅 등의 기능으로 확장하기도 좋은 구조입니다.
...
public class Client {
  public static void main(String[] args) {
    ...
    // 전등 켜기 명령 세팅 후 버튼 누르기 함수 호출
    remote.setCommand(lightOn);
    remote.pressButton();
    
    // 전등 끄기 명령 세팅 후 버튼 누르기 함수 호출
...
<mark>커맨드 패턴 예시 2 : 실행 취소 및 재실행</mark>
```
// 명령을 받는 리시버 클래스 : 텍스트 에디터
public class TextEditor {
  ...
  // 스트링빌더를 사용한 문자열 추가 함수
  public void insertText(String text, int position) {
    content.insert(position, text);
  }

  // 스트링빌더를 사용한 문자열 삭제 함수
  public void deleteText(int position, int length) {
    content.delete(position, position + length);
  }

  // 스트링빌더를 사용한 문자열 일부 반환 함수
  public String getTextSubstring(int start, int end) {
    return content.substring(start, end);
  }

  // 스트링빌더를 사용한 문자열 전체 반환 함수
  public String getContent() {
...
public class Main {
  public static void main(String[] args) {
    ...
    // 인보커를 통해 0번 위치부터 7글자 텍스트 삭제 명령 실행
...
특정 언어의 문법을 객체 구조로 표현하고 해석하여 결과를 만들어내는 패턴입니다.

복잡한 텍스트의 키워드, 연산자, 변수 등 문법 요소를 객체로 표현하고 해석하여 연산을 수행합니다.  
데이터베이스 쿼리, 스크립트 엔진, 파서, 수식 처리 프로그램 등에서 활용될 수 있습니다.

<mark>인터프리터 패턴 예시 1</mark>
```
// 표현식 인터페이스
public interface Expression {
  // 표현식이 나타내는 값을 반환하는 인터프리트 함수 정의
  int interpret();
}

// 표현식 인터페이스를 구현한 숫자 표현식 클래스
...
public class Add implements Expression {
  ...
  @Override
  public int interpret() {
      // 양쪽 표현식 결과 값을 더한 숫자 반환
      return leftExpression.interpret() + rightExpression.interpret();
  }
}

// 표현식 인터페이스를 구현한 빼기 표현식 클래스
public class Subtract implements Expression {
  ...
  @Override
  public int interpret() {
    // 왼쪽 표현식 결과 값에서 오른쪽 표현식 결과 값을 뺀 숫자 반환
    return leftExpression.interpret() - rightExpression.interpret();
  }
}
...
```
// 데이터베이스를 시뮬레이션하는 컨텍스트 클래스
class Context {
  // 데이터베이스 테이블 (맵, 리스트를 활용하여 구현)
  private Map<String, List<Map<String, String>>> tables;

  // 생성자에서 초기 샘플 테이터 생성 및 저장
  public Context() {
...
class WhereExpression implements Expression {
  // 컬럼, 연산자, 비교 값, 테이블명 필드
  private String column; // id, name, age 등
  private String operator; // >, =, < 등
  private String value;
  private String tableName;

  // 생성자에서 받은 컬럼, 연산자, 비교 값, 테이블명 저장
  ...
  // WHERE 조건을 충족하는 행 리스트를 반환하는 인터프리트 함수 구현
  @Override
  public List<Map<String, String>> interpret(Context context) {
    ...
    for (Map<String, String> row : table) {
      // 주어진 컬럼 값과 비교 값이 연산자 조건에 부합하면 결과 행 리스트에 추가
      if (evaluate(row.get(column), operator, value)) {
        result.add(row);
      }
    }

    // WHERE 표현식 해석 결과 행 리스트 반환
    return result;
  }

  // 주어진 컬럼 값과 비교 값이 연산자 조건에 부합하는지 여부 반환 함수
...
class SelectExpression implements Expression {
  // 조회할 테이블명, SELECT 대상 컬럼 리스트, WHERE 표현식 필드
  private String tableName;
  private List<String> columns;
  private Expression whereClause;

  // 생성자에서 받은 조회할 테이블명, SELECT 대상 컬럼 리스트, WHERE 표현식 저장
  ...
  // WHERE 조건을 충족하는 행 리스트에 SELECT 대상 컬럼들만 추출하여 반환하는 인터프리트 함수 구현
  @Override
  public List<Map<String, String>> interpret(Context context) {
    List<Map<String, String>> table = context.getTable(tableName);
    List<Map<String, String>> result = new ArrayList<>();
    ...
    for (Map<String, String> row : table) {
      ...
      // WHERE 표현식이 없거나, WHERE 표현식 결과 행 리스트가 비어있지 않은 경우
      if (whereClause == null || !whereClause.interpret(rowContext).isEmpty()) {
        Map<String, String> newRow = new HashMap<>();
        
        // SELECT 대상 컬럼들만 결과 행 리스트에 추가
...
// SQL 파서 클래스
class SQLParser {
  // 텍스트로 주어진 쿼리를 표현식 클래스들로 파싱하는 함수
  public static Expression parse(String query) {
    // 스페이스를 기준으로 쿼리 문자열 분리
    String[] parts = query.split("\\s+");

    // SELECT 문으로 시작하지 않는 경우, 예외 발생
...
public class Main {
  public static void main(String[] args) {
    // 컨텍스트 인스턴스 생성 (데이터베이스 역할)
    Context context = new Context();

    // 조건문이 없는 쿼리문을 파싱하여 SELECT 표현식 인스턴스 생성
    String query1 = "SELECT * FROM users";
    Expression expr1 = SQLParser.parse(query1);

    // 인터프리트 함수 호출하여 반환된 결과 행 리스트 출력
    ...
    // 조건문이 있는 쿼리문을 파싱하여 SELECT 표현식 인스턴스 생성
...
```
문자열로 주어진 SQL 쿼리를 파싱하고, 해석하여 처리하는 인터프리터 패턴 예시입니다.  
위 코드의 실행 결과는 아래와 같습니다.
```
...
{name=John, age=30}
```
HashMap은 순서를 보장하지 않아서 각 Map 내부 컬럼 출력 순서는 달라질 수 있습니다.

### 반복자 패턴 (Iterator)
컬렉션의 요소들을 순차적으로 탐색할 수 있도록 반복자를 제공하는 패턴입니다.  
컬렉션의 내부 구조를 직접 노출하지 않는다는 장점이 있습니다.

반복자 인터페이스를 활용하면 다양한 컬렉션을 공통된 방식으로 순회할 수 있습니다.  
많은 프로그래밍 언어의 컬렉션 순회 문법이나 제너레이터 등에서 이터레이터 개념이 활용됩니다.

<mark>반복자 패턴 예시 1</mark>
```
// 반복자 인터페이스
interface MyIterator {
  // 다음 요소가 있는지 확인하는 함수 정의
  boolean hasNext();

  // 다음 요소를 반환하고, 다음 위치로 이동하는 함수 정의
  Object next();
}
...
class MyList implements Collection {
  ...
  // 생성자에서 받은 사이즈 만큼의 리스트 배열 생성 및 저장
...
class MyListIterator implements MyIterator {
  ...
  // 생성자에서 리스트 및 초기 인덱스 저장
  public MyListIterator(MyList list) {
    this.list = list;
    this.index = 0;
  }

  // 다음 요소가 있는지 확인하는 함수 구현
  @Override
  public boolean hasNext() {
    // 현재 인덱스가 리스트 사이즈보다 작은지 여부 반환
    return index < list.size();
  }

  // 다음 요소를 반환하고, 다음 위치로 이동하는 함수 구현
  @Override
  public Object next() {
    // 리스트에 다음 요소가 남아있으면,
    if (this.hasNext()) {
      // 리스트에서 다음 요소 반환 후 그 다음 위치로 이동
...
public class Main {
  public static void main(String[] args) {
    ...
    // 반복자를 통해 리스트에 다음 요소가 있는지 확인하며, 요소가 남아있는 동안 반복
...
```
반복자 객체를 통해 컬렉션 내부 요소를 순차적으로 탐색하는 반복자 패턴 예시입니다.  
위 코드의 실행 결과는 아래와 같습니다.
```
A
B
...
class DepthFirstIterator implements FileSystemIterator {
  ...
  public FileSystemItem next() {
    ...
    if (current instanceof Directory) {
      ...
      // 폴더 내 파일시스템요소들을 역순으로 순회하며 스택에 추가
...
class BreadthFirstIterator implements FileSystemIterator {
  ...
  public FileSystemItem next() {
    ...
    // 파일시스템요소 큐에서 현재 요소 꺼내기
    FileSystemItem current = queue.poll();

    // 현재 요소가 폴더면,
    if (current instanceof Directory) {
      // 폴더 내 파일시스템요소들을 순서대로 큐에 추가
...
public class Main {
  public static void main(String[] args) {
    ...
    // 파일시스템 인스턴스 생성 및 루트 폴더 등록
    FileSystem fileSystem = new FileSystem(root);

    // 깊이 우선 순회 반복자 인스턴스를 통해 탐색하며 루트 폴더 내부 요소들 이름 출력
    ...
    // 너비 우선 순회 반복자 인스턴스를 통해 탐색하며 루트 폴더 내부 요소들 이름 출력
...
```
파일시스템 요소를 서로 다른 방식으로 순회하는 반복자를 구현하는 반복자 패턴 예시입니다.  
위 코드의 실행 결과는 아래와 같습니다.
```
...
### 메멘토 패턴 (Memento)
객체의 캡슐화를 유지하면서 특정 시점의 내부 상태를 스냅샷 형태로 외부에 저장하고,  
필요할 때 저장된 이전 상태로 복원할 수 있는 패턴입니다.

<mark>메멘토 패턴 예시 1</mark>
```
// 메멘토 클래스 : 특정 시점의 게임 진행 상태 (스냅샷)
class GameMemento {
  // 레벨 및 점수 필드
...
// 오리지네이터 클래스 : 현재 데이터로 메멘토 생성 및 데이터 복원
class Game {
  // 레벨 및 점수 필드
  ...
  // 메멘토 인스턴스 (스냅샷) 생성 및 반환 함수
  public GameMemento save() {
    return new GameMemento(level, score);
  }

  // 매개변수로 받은 메멘토 인스턴스에 저장된 데이터로 필드 값 복원 함수
...
public class Main {
  public static void main(String[] args) {
    ...
    // 게임 진행 상태 변경 후 오리지네이터를 통해 생성한 메멘토 인스턴스 (스냅샷) 을 케어테이커에 추가
    ...
    // 케어테이커에서 메멘토 인스턴스 (스냅샷) 을 가져와 오리지네이터를 통해 복원
...
class Document {
  ...
  // 현재 문서 내용을 메멘토 인스턴스로 생성 및 반환
  public DocumentMemento save() {
    return new DocumentMemento(content.toString());
  }

  // 매개변수로 받은 메멘토 인스턴스에 저장된 문서 내용 복원
...
class DocumentHistory {
  ...
  // 메멘토 스택에 메멘토 인스턴스 (스냅샷) 추가 함수
  public void push(DocumentMemento memento) {
    history.push(memento);
  }

  // 마지막으로 저장된 메멘토 인스턴스 반환 함수 : 이전 상태로 복원 시 사용
...
// 에디터 클래스 : 오리지네이터 및 케어테이커 활용하여 문서 수정 및 복원
class Editor {
  // 오리지네이터 및 케어테이커 필드
  private final Document document;
  private final DocumentHistory history;

  // 생성자에서 오리지네이터 및 케어테이커 인스턴스 생성
  public Editor() {
    this.document = new Document();
    this.history = new DocumentHistory();
  }

  // 오리지네이터를 통해 현재 메멘토 인스턴스를 생성하고 케어테이커에 저장 후 신규 텍스트 추가
  public void write(String text) {
    history.push(document.save());
    document.write(text);
  }

  // 케어테이커에 마지막으로 저장된 메멘토 인스턴스를 가져와 오리지네이터를 통해 복원 : 실행 취소 기능
  public void undo() {
...
public class Main {
  public static void main(String[] args) {
    ...
    // 케어테이커에 마지막으로 저장된 문서 내용으로 복원 및 출력 : 실행 취소
    editor.undo();
    System.out.println(editor.getContent());
...
<mark>문서 용량이 작고 수정이 빈번하지 않으면</mark>  
각 명령을 객체로 구현하는 커맨드 패턴보다 문서 전체 상태를 저장하는 메멘토 패턴 사용 시  
실행 취소 기능을 더 간단하게 구현할 수 있습니다.

<mark>문서 용량이 크거나 수정이 자주 발생하면</mark>  
상태를 저장할 때마다 문서 전체가 복사되므로 메모리 사용량과 처리 비용이 증가할 수 있습니다.

### 관찰자 패턴 (Observer)
관찰 대상 1 : 관찰자 N 관계에서 관찰 대상이 변경되면, 구독 중인 관찰자들이 자동으로 통지받는 패턴입니다.  
관찰자 추가 및 삭제가 용이하여, 동적인 일대다 관계를 유연하게 구현할 수 있습니다.

하나의 서브젝트 상태가 변경되거나 이벤트가 발생하면, 등록된 관찰자들에게 메시지를 보내는 형태입니다.
...
서브젝트가 옵저버를 직접 알고 변경 사항을 통지하는 관찰자 패턴과 달리,  
브로커 또는 메시지 채널 등 중간 매개체를 통해서 메시지를 주고받는 패턴입니다.

발행자가 브로커에게 메시지를 보내고, 브로커는 구독자들에게 전달하는 형태입니다.  
발행자와 구독자가 서로 직접 의존하지 않아 결합도가 낮고 비동기 처리 시 유리합니다.

복잡한 분산 시스템에서 컴포넌트 간 느슨한 결합을 유지하는 이벤트 중심 아키텍처를 구현할 수 있습니다.
...
class Broker {
  ...
  // 해당 주제 구독자 리스트에 구독자 추가 함수
  public void subscribe(String topic, Subscriber subscriber) {
    // 주제별 구독자 리스트 맵에 해당 주제 구독자 리스트가 없으면 생성 후 구독자 추가
    subscribers.computeIfAbsent(topic, k -> new ArrayList<>()).add(subscriber);
  }

  // 메시지 발행 함수 (발행자 클래스에서 호출 예정)
  public void publish(Message message) {
    // 해당 주제 구독자 리스트 가져오기
    List<Subscriber> topicSubscribers = subscribers.get(message.getTopic());
    
    // 해당 주제 구독자 리스트가 비어있지 않으면,
    if (topicSubscribers != null) {
      // 해당 주제 전체 구독자들의 메시지 수신 및 상태 업데이트 함수 호출
...
class EmailDeliveryService {
  ...
  // 비동기 작업을 실행할 스레드 풀 생성
  private ExecutorService executor = Executors.newCachedThreadPool();

  // 이벤트 종류에 해당하는 구독자 그룹에 구독자 추가 함수
  public void subscribe(String eventType, Customer customer) {
    // 이벤트 종류별 구독자 그룹 맵에 해당 이벤트 그룹이 없으면 생성 후 구독자 추가
  ...
  public void sendEmails(String eventType, String message) {
    // 이벤트 종류에 해당하는 구독자 그룹 가져오기
    List<Customer> customers = customerGroups.get(eventType);

    // 구독자 그룹이 비어있지 않으면,
    if (customers != null) {
      // 해당 이벤트 전체 구독자들의 메시지 수신 함수 호출
      for (Customer customer : customers) {
        // 각 구독자들에게 메인 스레드가 아닌 별도 스레드들에서 비동기적으로 이벤트 전달
...
public class Main {
  public static void main(String[] args) {
    ...
    // 발행자 인스턴스 생성 시 브로커 인스턴스, 이벤트 종류 저장
    ...
    // 브로커의 이벤트 캠페인 개시 함수 호출
    marketing.launchCampaign("New Product");

    // 신규 구독자 인스턴스 생성 후 브로커에 추가 등록
    ...
    try {
      // 이미 submit() 된 작업들이 완료될 시간을 주기 위해 메인 스레드 대기
      Thread.sleep(10000);
...
```
발행자가 메시지를 브로커에게 전달하고, 브로커가 구독자들에게 비동기적으로 메시지를 전달하여  
시간이 소모되는 작업으로부터 발행자를 분리할 수 있는 발행-구독 패턴 예시입니다.  
위 코드의 실행 결과는 아래와 같습니다.
```
...
```
비동기적으로 처리하므로, 첫 번째 이벤트 구독자 처리 완료 전에 두 번째 이벤트 캠페인이 개시될 수 있습니다.  
각 구독자의 메시지 처리 작업이 서로 다른 스레드에서 실행되고 실행 순서가 보장되지 않기 때문에,  
메시지 수신 및 처리 완료 순서는 실행할 때마다 달라질 수 있습니다.
...
객체 구조는 안정적이지만 객체에 수행할 기능이 자주 추가/변경되는 상황에서 유용하게 사용되는 패턴입니다.

기존 클래스에 방문자 객체의 visit() 함수를 호출하는 accept() 함수를 미리 추가해두면,  
기존 클래스를 변경하지 않고 방문자 클래스를 통해 새로운 기능을 추가할 수 있습니다.
...
class Circle implements Shape {
  ...
  public void accept(Visitor visitor) {
    // 현재 클래스를 인자로 방문자 함수 호출
    visitor.visit(this);
  }
}

// 모양 인터페이스를 적용한 직사각형 클래스
...
class SizeCalculatorVisitor implements Visitor {
  ...
  // 폴더 방문하여 폴더 내 요소들 크기 측정
  @Override
  public void visit(Directory directory) {
    for (FileSystemElement element : directory.getElements()) {
      // 방문자 수용 함수를 호출하여 재귀적으로 방문
      element.accept(this);
    }
  }
...
public class Main {
  public static void main(String[] args) {
    ...
    // 루트 폴더 방문하여 재귀적으로 파일 크기 합산
    rootDir.accept(sizeVisitor);
    // 루트 폴더 내 모든 파일 크기 반환하여 출력
...
### 명세 패턴 (Specification)
객체의 속성이나 동작이 특정 조건들을 만족하는지 여부를 확인할 때 유용한 패턴입니다.

여러 조건들을 재사용 가능한 객체로 만들고, 조합해서 다양한 새로운 조건을 만들어낼 수 있습니다.

<mark>명세 패턴 예시 1</mark>
```
// 명세 인터페이스
public interface Specification {
  // 숫자가 특정 조건을 만족하는지 여부 반환 함수 정의
  boolean isSatisfiedBy(int number);

  // 다른 명세 인스턴스를 조합하여 새로운 명세 인스턴스를 반환하는 함수
  // default 메서드라서 인터페이스에서도 내부 구현 가능
  default Specification and(Specification other) {
    // 숫자가 두 명세 조건을 모두 만족하는지 여부 반환
    return number -> this.isSatisfiedBy(number) && other.isSatisfiedBy(number);
  }
}

// 명세 인터페이스를 구현한 짝수 명세 클래스
class EvenSpecification implements Specification {
  // 숫자가 특정 조건을 만족하는지 여부 반환 함수 구현
  @Override
...
class RangeSpecification implements Specification {
  ...
  @Override
  public boolean isSatisfiedBy(int number) {
    // 숫자가 최소값보다 크거나 같고, 최대값보다 작거나 같으면 true 반환
...
// 명세 인터페이스
public interface Specification {
  // 주어진 제품이 특정 조건을 만족하는지 여부 반환 함수 정의
  boolean isSatisfiedBy(Product item);
}

// 명세 인터페이스를 구현한 카테고리 명세 클래스
...
public class PriceSpec implements Specification {
  ...
  @Override
  public boolean isSatisfiedBy(Product item) {
    // 제품이 기준 가격 이하면 true 반환
    return item.getPrice() <= maxPrice;
  }
}

// 명세 인터페이스를 구현한 재고 유무 클래스
public class InStockSpec implements Specification {
  // 주어진 제품이 특정 조건을 만족하는지 여부 반환 함수 구현
  @Override
  public boolean isSatisfiedBy(Product item) {
    // 제품 재고가 남아있으면 true 반환
    return item.getStock() > 0;
  }
}

// 명세 인터페이스를 구현한 명세 AND 조합 클래스
...
public class OrSpec implements Specification {
  ...
  @Override
  public boolean isSatisfiedBy(Product item) {
    // 제품이 두 명세 조건 중 하나라도 만족하면 true 반환
    return spec1.isSatisfiedBy(item) || spec2.isSatisfiedBy(item);
  }
}

// 명세 인터페이스를 구현한 명세 NOT 변형 클래스
...
public class ProductFilter {
  // 제품 리스트 요소 중 명세에서 참을 반환하는 요소 필터 함수
  public static List<Product> filter(List<Product> items, Specification spec) {
    // 조건을 만족하는 요소들로 신규 제품 리스트 생성 및 반환
    return items.stream()
                .filter(spec::isSatisfiedBy)
                .collect(Collectors.toList());
  }

  // 매개변수로 받은 제품 리스트 내 요소들 출력 함수
...
public class Main {
  public static void main(String[] args) {
    ...
    // 카테고리 명세, 재고 유무, 가격 명세 인스턴스 생성
    Specification electronicsSpec = new CategorySpec("Electronics");
    Specification inStockSpec = new InStockSpec();
    Specification affordableSpec = new PriceSpec(500);

    // AND 조합 명세, OR 조합 명세, NOT 변형 명세 인스턴스 생성
    ...
    Specification expensiveSpec = new NotSpec(affordableSpec);

    // 카테고리가 Electronics이고 재고가 남아있는 제품들 필터링 및 출력
    System.out.println("Electronics in stock:");
    ProductFilter.printProducts(ProductFilter.filter(products, electronicInStock));

    // 카테고리가 Electronics이거나 재고가 남아있는 제품들 필터링 및 출력
    System.out.println("\nElectronics or items in stock:");
    ProductFilter.printProducts(ProductFilter.filter(products, electronicOrInStock));

    // 기준 가격보다 높은 제품들 필터링 및 출력
    System.out.println("\nExpensive items:");
    ProductFilter.printProducts(ProductFilter.filter(products, expensiveSpec));
  }
}
```
첫 번째 예시보다 다양한 조합이나 변형을 사용하는 명세 패턴 예시입니다.  
조합 및 변형 클래스들을 분리해두면 다른 조합 연산들도 유연하게 추가할 수 있습니다.  
위 코드의 실행 결과는 아래와 같습니다.
```
Electronics in stock:
Laptop - Electronics - $1200 - Stock: 5
Headphones - Electronics - $200 - Stock: 10

Electronics or items in stock:
Laptop - Electronics - $1200 - Stock: 5
Smartphone - Electronics - $800 - Stock: 0
Headphones - Electronics - $200 - Stock: 10
Book - Literature - $20 - Stock: 50
