...
### 파사드 패턴
여러 개의 복잡한 서브시스템 클래스들을 하나의 파사드 클래스로 묶어서,  
클라이언트가 서브시스템을 직접 호출하지 않고 파사드 클래스만 사용하여 쉽게 기능을 이용하도록 하는 패턴입니다.
...
public class Main {
  public static void main(String[] args) {
    ...
    smartHome.wakeUp();    // 아침 루틴 실행
    smartHome.leaveHome(); // 외출 루틴 실행
  }
}
```
...
public class Main {
  public static void main(String[] args) {
    ...
    fs.writeFile("test.txt", "Hello, Facade Pattern!");
    System.out.println("File content: " + fs.readFile("test.txt"));
    System.out.println("File delete success: " + fs.deleteFile("test.txt"));
  }
}
```
