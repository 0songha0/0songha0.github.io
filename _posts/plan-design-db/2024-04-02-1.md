...
3. INSERT SELECT문으로 실제 테이블에 마이그레이션 테이블 데이터를 일괄 업로드합니다.
...
이미 내보내기 한 테이블 CSV가 있다면, 재사용 하면 됩니다.

### CSV 데이터 채우기
업로드용으로 전달받은 엑셀 내용으로 마이그레이션 테이블 컬럼에 맞춰서 내용을 채웁니다.  
Autoincrement로 자동 증가되지 않는 PK라면, MAX 값 조회해서 +1부터 채웁니다.

### CSV 파일 인코딩 변환
csv 파일을 메모장으로 열고, 다른 이름으로 저장 > 인코딩: UTF-8 선택 후 저장합니다.  

엑셀에서 CSV 파일을 저장하면 인코딩이 ANSI이기 때문에,  
그대로 가져오면 DB 인코딩 UTF-8과 달라서 한글이 전부 깨질 수 있습니다.
