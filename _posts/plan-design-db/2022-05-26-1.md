...
<table class="table_4_left">
  ...
  <tbody>
    <tr>
      ...
      <td>
        PK<br/>
        Auto Increment, UUID 등 변경되지 않는 고유 식별값을 사용하는 것이 권장됩니다.<br/>
        바뀔 가능성이 있는 이메일, 개인정보인 주민등록번호 등은 PK로 지정하지 않는 것이 좋습니다.<br/>
        <br/>
        단일 DB에서는 Auto Increment가 저장 공간과 인덱스 크기가 작고, 성능이 유리해서 많이 쓰입니다.<br/>
        MSA 등 분산시스템이나 ID를 미리 생성해야 하는 경우에는 UUID가 유리하지만,<br/>
