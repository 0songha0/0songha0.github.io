...
### 쿼리 성능 개선 방법
WhaTap 같은 서버 모니터링 툴에서 오래 걸리는 쿼리를 확인하고 아래와 같이 개선할 수 있습니다.

<mark>실행계획에서 type이 ALL(풀스캔)이고 rows가 많은데 오래걸리는 경우</mark>
WHERE 조건에 사용된 해당 테이블 컬럼에 인덱스를 생성하면 쿼리 성능이 개선됩니다.

<mark>조건절에서 서브쿼리 사용한 경우</mark>
```
DELETE
FROM
	im_tstp_qstn_ai_ans
...
아래처럼 JOIN으로 변경하면 더 적은 리소스를 사용하여 성능이 향상됩니다.
```
DELETE a FROM im_tstp_qstn_ai_ans a 
JOIN im_evl_trgt_ai b ON a.evl_trgt_ai_id = b.evl_trgt_ai_id 
JOIN im_evl_trgt c ON b.evl_trgt_id = c.evl_trgt_id 
WHERE c.evl_ditb_id = '키값';
```
조인 쿼리로 변경 후 아래와 같은 작업이 필요합니다.
- im_evl_trgt_ai 테이블의 evl_trgt_ai_id와 evl_trgt_id 인덱스 추가
- im_evl_trgt 테이블의 evl_trgt_id와 evl_ditb_id 열 인덱스 추가
