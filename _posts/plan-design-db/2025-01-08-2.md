...
<mark>컬럼 내 JSON 값 변경</mark>
```
UPDATE 테이블명
SET FILE_STNG = JSON_SET(컬럼명, '$.JSON키', '값')
WHERE 검색조건
```
...
### 사용자 로그테이블 마지막 사용시간 조회
```
SELECT MAX(사용시간컬럼명) AS last_use_time
FROM 사용자로그테이블명
WHERE 사용자ID컬럼명 = '사용자ID';
```
위 쿼리는 정렬을 하지 않아서, 인덱스가 없는 테이블이면 ORDER BY + LIMIT 1 을 사용한 쿼리보다 훨씬 빠릅니다.