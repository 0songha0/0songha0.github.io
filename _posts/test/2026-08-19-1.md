...
```
현재 EC2 서버에서 CloudWatch Agent 실행 명령어를 입력하여 실행합니다.
...
top
```
EC2 서버에 연결해서 위 명령어를 입력하면, 서버 자원을 실시간으로 확인할 수 있습니다.  
상단 %Cpu(s)가 현재 CPU 사용률이고 avail Mem이 사용 가능한 메모리입니다.
...
### EC2 모니터링 위젯에 이중화 서버 추가 방법
<mark>EC2 CPU 사용률 위젯에 이중화 서버 추가</mark>  
AWS > CloudWatch 검색 > 대시보드 메뉴 > 대시보드 선택 > 기존 EC2 CPU 사용률 위젯 편집 > 찾아보기 탭 > CPUUtilization 검색 > 'EC2 > 인스턴스별 지표' 클릭 > 기존 EC2 서버 기반으로 복제된 backend-server2 서버 추가 선택 > 그래프로 표시된 지표 탭에서 기간 : 1분 확인 > 업데이트 > 저장

<mark>EC2 메모리 사용률 위젯에 이중화 서버 추가</mark>  
AWS > CloudWatch 검색 > 대시보드 메뉴 > 대시보드 선택 > 기존 EC2 메모리 사용률 위젯 편집 > 찾아보기 탭 > mem_used_percent 검색 > 'CWAgent > InstanceId' 클릭 > 기존 EC2 서버 기반으로 복제된 backend-server2 서버 추가 선택 > 그래프로 표시된 지표 탭에서 기간 : 1분 확인 > 업데이트 > 저장
