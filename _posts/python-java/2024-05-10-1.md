...
<mark>재귀 및 스택으로 트리 탐색 DFS 구현 예시</mark>
```
// 트리 구성용 노드 클래스
class Node {
  int data;
...
한 번에 1씩 이동하여 가중치가 없는 트리 및 그래프에서  
레벨 탐색, 최소 이동 횟수, 최단거리 문제는 BFS로 풀어야 효율적입니다.
...
public int BFS(int start, int end) {
  ...
  // 큐가 비어있지 않을 때까지 반복 ★
  while(!queue.isEmpty()) {
    // 현재 큐 사이즈 저장 (큐에서 원소 꺼내면 값이 변경되므로 따로 저장) ★
    int len = queue.size();
...
<mark>우선순위 큐, 노드로 다익스트라 구현 예시</mark>
```
// 노드 클래스 정의
...
역 번호가 많은 최소 환승 경로 문제에서는 인접행렬보다 해시맵에 저장하는 것이 메모리 효율적입니다.

<mark>우선순위 큐, 배열로 다익스트라 구현 예시</mark>
```
public int solution(int[][] board) {
  ...
  // i행 j열까지 가는데 부숴야 하는 벽의 최소 개수 2차원 배열
  ...
  // 부숴야 하는 벽의 개수 기준으로 오름차순 정렬하는 우선순위 큐 생성
  ...
  while (!pq.isEmpty()) {
    ...
    // 현재 지점의 부숴야 하는 벽의 개수가
    // 기존 현재 지점의 부숴야 하는 벽의 개수보다 크면 지나가기 (★ 시간복잡도에 중요한 영향)
    if (cur[2] > cost[cur[0]][cur[1]]) continue;

    // 현재 지점에서 다음 지점들 방향 탐색
    for (int k = 0; k < 4; k++) {
      int nx = cur[0] + dx[k];
      int ny = cur[1] + dy[k];
      ...
      if (board[nx][ny] == 0 && cost[nx][ny] > cur[2]) {
        // 다음 지점이 갈 수 있는 곳이고,
        // 다음 지점의 부숴야 하는 벽의 개수가
        // 기존 다음 지점의 부숴야 하는 벽의 개수보다 작으면
...
```
출발지점 { 0, 0 } 부터 도착지점 { n-1, m-1 } 까지 부숴야 하는 벽의 최소 개수를 반환하는 풀이입니다.
...
### 위상 정렬 알고리즘
사이클이 없는 방향 그래프에서 간선 방향 및 선후 관계를 유지하며 정렬하는 알고리즘입니다.  
일련의 작업들을 순차적으로 수행하기 위해 사용하기도 합니다.
...
### 백트래킹
가능한 모든 경우의 수를 탐색하는 완전탐색의 한 종류입니다.  
답이 될 가능성이 없는 노드에선 탐색을 포기하고 되돌아가는 방식입니다.  
...
앞 작은 크기의 문제 해를 이용해서 다음 크기의 문제를 해결할 수 있습니다.

특정 합을 만드는 경우의 수 문제 등에서 DP를 사용하면 효율적입니다.

DP 테이블의 i번째 또는 [i][j]번째 값의 의미를 잘 정의해둬야 점화식을 만들 수 있습니다.
...
3. 메모이제이션 기법으로 작은 문제의 결과를 저장하여 중복 계산을 피합니다.

### 냅색 알고리즘
제한된 조건을 충족하는 조합들 중에서 최대 가치를 구하는 DP 기반 알고리즘입니다.  

백준 동전1, 동전2, 평범한 배낭은 전형적인 냅색 알고리즘 문제들입니다.  
동전 단위가 소수점 둘째자리로 들어오면, x 100 해서 정수로 바꾸고 DP 테이블을 만들어야 합니다.

요소가 무한개 있어서 중복 사용이 가능하면 왼쪽 → 오른쪽으로 for문을 돌려야 하고,  
요소당 1개만 사용 가능하면 오른쪽 → 왼쪽으로 for문을 돌려야 합니다.

<mark>거스름돈 동전 개수 문제 풀이</mark>
```
public int solution(int[] coin, int n) {
  // DP 테이블 초기화
  int[] dy = new int[n + 1]; // 각 금액마다 필요한 최소 동전 수 배열
...
그리디 알고리즘 문제인지는 증명하기 어려우므로, 오답이 될만한 반례를 만들어 검증하는 것이 좋습니다.

<mark>그리디로 풀만한 문제 유형</mark>
- N명 이하, 총 140kg 이하만 탈 수 있는 보트로 사람들을 옮길 때 최소 횟수
- 수직선 상에서 범위로 물을 줄 때, 작동시킬 스프링쿨러 최소 개수
...
결정 알고리즘은 문제가 요구하는 정답이 반드시 범위 안에 존재하고,  
정답 후보값에 대한 판별 결과가 (YES/NO) 처럼 단조성을 가지는 경우에만 사용 가능합니다.
...
public int solution(int n, int[] stables) {
  ...
  while (lt <= rt) {
    ...
    if (count(stables, mid) >= n) { // 단조성 (말 배치 가능 YES / 불가능 NO)
...
<mark>나선형 좌석 배치 시뮬레이션 문제 풀이</mark>
```
