...
그래프 인접 행렬 + for문을 이용하면 방문하지 않은 각 노드마다 최단거리 노드를 순차 탐색해서 O(n²)이지만,  
그래프 인접 리스트 + 우선순위 큐를 이용하면 다음 최단거리 후보 노드를 바로 꺼낼 수 있어 효율적입니다.

<mark>우선순위 큐로 다익스트라 구현 예시</mark>
```
// 노드 번호, 현재 노드까지의 가중치 저장 클래스 생성
...
class Main {
  public static void main(String args[]) {
    ...
    Main main = new Main();
    main.solution(graph, dist, visited);

    // 출발 노드부터 각 노드까지의 최단거리 출력
    for (int i = 1; i <= n; i++) {
  ...
  public void solution(List<List<Node>> graph, int[] dist, boolean[] visited) {
    ...
    while(!pq.isEmpty()) {
      ...
      // Node 클래스 내 compareTo 메소드 기준으로 정렬되어 꺼내짐
      Node curNode = pq.poll();

      // 현재 노드를 이미 방문했다면 넘어가기
      if (visited[curNode.vertex]) continue;
      
      // 현재 노드 방문 처리 (현재 노드 중복 처리 방지)
...
간선이 적으면 현재 노드에서 갈 수 있는 노드만 저장하는 인접 리스트로 저장하는 것이 좋습니다.
