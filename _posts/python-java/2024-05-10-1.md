...
### 다익스트라
양의 가중치만 있는 방향/무방향 그래프에서 최단 경로를 우선순위 큐로 구하는 알고리즘입니다.  
...
<mark>우선순위 큐로 다익스트라 구현 예시</mark>
```
// 도착 노드, 가중치 겂을 저장할 수 있는 간선 객체 생성
...
class Main {
  ...
  ` int[] dist = new int[n+1];
    ...
    // 출발 노드부터 목표 노드까지 최단거리 탐색
    main.solution(n, m, graph, dist);

    // 각 노드까지의 최소 가중치 출력
    for (int i = 1; i <= n; i++) {
      if (dist[i] != Integer.MAX_VALUE) {
        System.out.println(i + "번 노드 최단거리 : " + dist[i]);
      }
    }
  }

  // 다익스트라 알고리즘으로 출발 노드부터 목표 노드까지 최단거리 탐색 함수
  public void solution(int n, int m, List<List<Edge>> graph, int[] dist) {
    PriorityQueue<Edge> pq = new PriorityQueue<>();

    // 출발 노드 간선 및 최소 가중치 추가
    pq.offer(new Edge(1, 0));
    dist[1] = 0;

    // 큐가 비어있지 않을 때까지 반복
    while(!pq.isEmpty()) {
      // 현재까지 남은 간선 중, 가장 가중치가 작은 간선 꺼내기
      // Edge 객체 내 compareTo 메소드 기준으로 정렬되어 꺼내집니다.
      Edge curEdge = pq.poll();

      // 현재 간선의 다음 노드로 가는 최소 가중치보다 현재까지의 간선 가중치가 크면 넘어가기
      if(dist[curEdge.nextVertex] < curEdge.weight) {
        continue;
      }

      // 현재 간선의 다음 노드에 연결된 다음 간선들 우선순위 큐에 추가
      for(Edge nextEdge : graph.get(curEdge.nextVertex)) {
        
        // 다음 간선의 다음 노드로 가는 최소 가중치보다 현재까지의 간선 가중치 + 다음 간선 가중치가 작으면
        if(dist[nextEdge.nextVertex] > curEdge.weight + nextEdge.weight) {
          // 다음 노드로 가는 최소 가중치 갱신
          dist[nextEdge.nextVertex] = curEdge.weight + nextEdge.weight;

          // 다음 노드로 가는 최소 가중치 간선 추가
          pq.offer(new Edge(nextEdge.nextVertex, curEdge.weight + nextEdge.weight));
        }
      }
    }
  }
...
```
가중치 그래프에서 출발 노드부터 목표 노드까지 가는 최단거리를 찾습니다.  
for문을 이용하면 방문하지 않은 각 노드마다 최단거리 노드를 순차 탐색해서 O(n²)이지만,  
우선순위 큐를 이용하면 다음 최단거리 노드 후보를 바로 꺼낼 수 있어 효율적입니다.
