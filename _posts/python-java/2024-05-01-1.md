...
```
Integer.compare 사용 시, 계산 결과가 int 범위를 초과하는 경우 overflow 에러를 방지할 수 있습니다.  
오름차순 시에는 Integer.compare(this.x, order.y); 를 리턴하면 됩니다.

### 정렬 기준 커스텀 클래스 활용 방법
```
// ArrayList 정렬
ArrayList<클래스명> list = new ArrayList<>();
list.add(new 클래스명(1, 5));
list.add(new 클래스명(2, 3));
Collections.sort(list); // list 내부 클래스의 compareTo 함수 기준 정렬
...
```
Comparable 인터페이스를 상속받고, compareTo 함수를 재정의 한 클래스를 활용하여 정렬하는 방법들입니다.
