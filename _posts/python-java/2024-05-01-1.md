...
### 배열 (Array)
메모리에 연속적으로 저장되는 배열은 크기 변경이 불가하므로, 저장할 데이터 수가 고정된 경우 사용합니다.
...
원소 추가/삭제는 앞/뒤 원소 포인터를 변경하면 되어서 빠르지만, 해당 위치까지 순회해서 O(n) 입니다.
```
// 연결리스트 객체 생성
...
if (set.contains(i)) { // TreeSet은 자가균형 이진탐색트리로 이루어져 있으므로, O(logn) 입니다.
