...
### 연결리스트 (LinkedList)
메모리에 불연속적으로 저장되지만, 각 노드에 값과 다음 노드 주소 포인터를 저장하여 연속성을 가지는 자료구조입니다.  
선언 시 메모리를 할당하는 Array와 달리, 데이터가 추가되는 시점에 할당하여 메모리 효율적입니다.
...
### 힙 (Heap)
빠르게 최소값 또는 최대값을 추출할 수 있는 완전 이진 트리 자료구조입니다.  
...
<mark>HashMap</mark>  
키-값 형태로 데이터를 저장하며, 키가 같으면 덮어써져 중복이 제거되고 순서가 보장되지 않는 자료구조입니다.
...
<mark>TreeMap</mark>  
키-값 형태로 데이터를 저장하며, 키가 같으면 덮어써져 중복이 제거되고 정렬 조건에 맞게 정렬되는 자료구조입니다.
```
...
<mark>TreeSet</mark>  
이진 탐색 트리 기반으로 구현되어 있으며, 중복을 제거하고 정렬 조건에 맞게 정렬합니다.
```
// 중복값 제거, 오름차순 정렬 세팅 (기본값)
