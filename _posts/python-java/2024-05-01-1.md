...
// 입력받은 소수 둘째 자리까지 정수로 만들기
// 예시 : 8.24 -> 824
int v = (int) Math.round(sc.nextDouble() * 100);
// Math.round를 이용해 부동소수점 오차를 보정합니다.
...
// 프리미티브 타입 배열 char[]를 문자열로 변환
char[] chars = {'H', 'e', 'l', 'l', 'o'};
String str = String.valueOf(chars);
// 객체 타입 배열 Character[]는 String.valueOf로 문자열 변환이 안되므로,
// char[] 등으로 변환 후 처리 필요
...
// 앞에서부터 처음 일치한 문자열 하나만 변경
str = str.replaceFirst("문자열1", "문자열2");

// 일치하는 문자열 전체 변경
str = str.replace("문자열1", "문자열2");
...
// 정규표현식으로 l~z에 해당하지 않는 문자 모두 제거
str = str.replaceAll("[^l-z]", "");

// 문자열을 n번 반복한 문자열 생성
str = str.repeat(n);

// 0번째 인덱스부터 5번째 앞까지 문자열 추출
str = str.substring(0, 5);

// 7번째 인덱스부터 끝까지 문자열 추출
str = str.substring(7);
...
public class Solution {
  public static void main(String[] args) {
    ...
    // 2차원 배열 모든 원소 값을 최대값으로 채우기
    for (int i = 0; i < arrs.length; i++) {
      Arrays.fill(arrs[i], Integer.MAX_VALUE);
    }
    ...
    // 오름차순 정렬된 프리미티브 타입 배열 int[]를 객체 타입 배열 Integer[]로 변환 후 내림차순
    ...
    // 내림차순 정렬 방법 2
    // 객체 타입만 정렬 가능한 Collections.reverseOrder() 대신 람다 활용
    Arrays.sort(Integer_arr, (a, b) -> Integer.compare(b, a));

    // 내림차순 정렬 방법 3 (가장 효율적)
    // 오름차순 정렬된 배열에서 투 포인터로 앞/뒤 인덱스 교환
    ...
    // 2차원 배열 정렬
    Arrays.sort(arrs, (a, b) -> Integer.compare(a[0], b[0]));
    // 2차원 배열 내 0번째 요소 기준으로 오름차순 정렬

    // 객체 배열 정렬
    Arrays.sort(배열, (객체명 o1, 객체명 o2) -> Integer.compare(o1.int변수명, o2.int변수명));
    // 배열에 저장된 객체의 int 변수 값을 람다 함수로 받아서 비교 후 오름차순 정렬합니다.
    // o1.int변수명 > o2.int변수명이면, 양수를 반환하여 o1이 o2보다 뒤에 오도록 정렬됩니다.
    
    // 배열 복사 ★
    int[] arr2 = arr.clone();
    또는
    arr2 = Arrays.copyOf(arr, arr.length); // 길이 변경 가능

    // 배열 부분 복사
    int[] arr2 = new int[arr.length+1];
    System.arraycopy(arr, 0, arr2, 1, arr.length); // 인덱스 0번째를 1번째로 복사
...
public class Solution {
  public static void main(String[] args) {
    ...
    list.sort((a, b) -> a[0] != b[0] ? Integer.compare(a[0], b[0]) : Integer.compare(b[1], a[1]));
    // 첫번째 요소 기준으로 오름차순 후 두번째 요소 기준으로 내림차순
...
// 연결리스트 오름차순 정렬 : O(nlogn)
Collections.sort(list);

// 연결리스트 0번째 원소 제거 후 반환 : O(1)
list.pollFirst();

// 연결리스트 마지막 원소 제거 후 반환 : O(1)
list.pollLast();

// 연결리스트에서 값과 처음으로 일치하는 중간 원소 삭제 : O(n)
list.remove("값");
...
// 연결리스트가 비어있지 않으면, 0번째 요소 값 확인 및 비교
if (!list.isEmpty() && "비교값".equals(list.peek())) {
...
// { 현재노드, 다음노드, 가중치 } 값을 int 배열로 구성하였을 때,
// 간선 추가 시, 가중치 낮은 순서로 오름차순 정렬하는 우선순위 큐 객체 생성
...
// 컬렉션 객체의 모든 데이터를 우선순위 큐 객체에 삽입 : O(nlogn)
// (n개 삽입 * logn 재정렬)
pq.addAll(list);
...
list.get(i);
// 위와 같이, set을 리스트로 변환하면 인덱스로 접근이 가능합니다.
```
...
<mark>이진 탐색 트리 (Binary Search Tree)</mark>  
...
<mark>인접 리스트 그래프 구현 (int[])</mark>
```
List<List<int[]>> graph = new ArrayList<>();

// i번째 List 꺼내서 int 배열 원소 추가
list.get(인덱스).add(new int[]{ nextVertex, cost });

// i번째에서 갈 수 있는 다음 정점들 순회
...
<mark>인접 리스트 그래프 구현 (Map-Set)</mark>
```
Map<Integer, Set<Integer>> graph = new HashMap<>();

// Map에 키가 없으면 키 생성 후 Set 저장
graph.putIfAbsent("키", new HashSet<>());

// Map에서 키에 해당하는 Set 꺼내서 원소 추가
graph.get("키").add("값");

// 현재 키에 해당하는 Set 원소 순회
for (int line : graph.get("키")) {
  
}
```
가중치가 없는 버스 및 지하철 호선 문제에서 사용할 수 있습니다.
...
또는
클래스list.sort(null); // Comparator 매개변수를 전달하지 않고, 기본 정렬 사용
```
