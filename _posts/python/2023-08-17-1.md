...
이 글은 파이썬 입문자인 저의 프로그래머스 문제 풀이 코드와 본받을 코드를 모아서 복습하는 글입니다.
...
문자열 연산, 배열, 조건문, 반복문 등 기본적인 문법을 익힐 수 있는 문제들입니다.  
...
def solution(a, b, c, d):
    ...
    if max(counts) == 4:
        ...
        return min(nums)
이 문제는 풀지 못해서 풀이 메모해두고 다시 볼 예정이다.
