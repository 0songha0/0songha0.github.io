...
### 연결리스트 (LinkedList)
원소가 데이터-포인터 쌍으로 구성되어 있으며, 포인터는 다음 원소의 메모리 위치를 가리킵니다.  
데이터가 메모리 상 흩어져 저장되어, 앞 원소부터 순차적으로 탐색해야 데이터에 접근할 수 있습니다.  
데이터 추가는 추가할 위치의 앞/뒤 원소 포인터를 변경만 하면 되어서 O(1)으로 간단합니다.
```
// 연결리스트 객체 생성
...
### 해시 (Hash)
키를 해시 함수를 통해 변환한 해시 값 인덱스에 값을 저장하여 검색/삽입/삭제가 빠른 자료구조입니다.  
