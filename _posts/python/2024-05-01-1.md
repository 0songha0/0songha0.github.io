...
<mark>LinkedHashSet</mark> : 중복을 제거하고 데이터를 추가한 순서대로 정렬됩니다.
```
// LinkedHashSet 생성
...
<mark>TreeSet</mark> : 이진 검색 트리 기반으로 구현되어 있으며, 중복을 제거하고 정렬 조건에 맞게 정렬됩니다.
```
