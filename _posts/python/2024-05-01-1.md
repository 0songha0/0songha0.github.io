...
빠르게 최소값/최대값을 추출할 수 있으며, 중간 노드는 추출할 수 없습니다.

<mark>최소힙 원소 추가 예시</mark>
1. 추가한 수를 힙의 가장 후미에 저장합니다.
2. 부모의 수가 자식의 수보다 크면 수를 교환합니다.
3. 부모의 수가 자식의 수보다 크지 않을 때까지 부모와 비교, 교환을 반복합니다.

<mark>최소힙 원소 삭제 예시</mark>
1. 가장 위에 있는 root 노드를 삭제합니다.
2. 가장 후미에 있는 수를 root 노드로 이동합니다.
3. root 노드의 수가 자식의 수보다 작은 경우, 더 작은 자식과 위치를 교환합니다.
4. 부모의 수가 자식의 수보다 크지 않을 때까지 자식와 비교, 교환을 반복합니다.
