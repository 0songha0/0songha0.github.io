...
모든 노드의 최대 차수(하위 간선 수)가 2인 트리를 이진 트리라고 합니다.

### 그래프 (Graph)
노드와 방향/무방향 간선을 이용한 비선형 자료구조입니다.  
간선에는 가중치가 존재할 수 있습니다.

<mark>깊이 우선 탐색 (DFS)</mark>  
시작 노드부터 간선을 따라 깊은 노드까지 이동하고, 이전에 방문한 노드로 거슬러 올라가며 연결된 노드 중 방문하지 않은 노드를 다시 최대 깊이까지 차례대로 방문합니다.  
스택 또는 재귀를 사용하여 DFS를 구현할 수 있습니다.  
최단 경로를 찾는 문제가 아니면 깊이 우선 탐색을 우선 고려해보는 것이 좋습니다.

<mark>너비 우선 탐색 (BFS)</mark>  
시작 노드와 간선의 차수가 가장 가까운 노드를 우선하여 방문하는 알고리즘입니다.  
큐에 add 하여 방문 처리 하면서 구현할 수 있습니다.  
