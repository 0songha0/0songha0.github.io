...
```
// 오름차순 정렬하는 우선순위 큐 객체 생성
PrioriryQueue<Integer> pq = new PriorityQueue<>();

// 내림차순 정렬하는 우선순위 큐 객체 생성
PrioriryQueue<String> pq = new PriorityQueue<>(Collections.reverseOrder());

// 노드의 cost 값에 따라 오름차순 정렬하는 우선순위 큐 객체 생성
PrioriryQueue<String> pq = new PriorityQueue<>((o1, o2) -> Integer.compare(o1.cost, o2.cost));

// 컬렉션의 모든 데이터를 담은 우선순위 큐 객체 생성
PrioriryQueue<String> pq = new PriorityQueue<>(list);

// 컬렉션의 모든 데이터를 우선순위 큐 객체에 삽입 : O(nlogn)
pq.addAll(list);
...
### 해시 (Hash)
키를 해시 함수를 통해 변환한 해시 값을 인덱스로 값을 저장해서 검색이 빠른 자료구조입니다.  
...
이진 트리는 모든 노드의 최대 차수(하위 간선 수)가 2인 트리입니다.
...
현재 노드에서 방문할 수 있는 아직 방문하지 않은 노드 중, 최소 비용이 가장 적은 노드를 선택합니다.  
