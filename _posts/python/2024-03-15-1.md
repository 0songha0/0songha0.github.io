...
class Solution {
  ...
  class Node {
    ...
    public Node(String next_word, int change_cnt) {
  ...
  public int solution(String begin, String target, String[] words) {
    ...
    while (!queue.isEmpty()) {
      ...
      // 현재 단어가 목표 단어와 같으면
      if (cur.next_word.equals(target)) {
        // 현재까지의 변환 횟수 저장
        answer = cur.change_cnt;
        break;
      }

      // 단어 리스트를 순회하며 변환 가능한 단어 탐색
      for (int i = 0; i < n; i++) {
        // 방문하지 않은 단어이고, 현재 단어에서 다음 단어를 한 글자만 바꿔서 만들 수 있다면
        if (!visited[i] && isNext(cur.next_word, words[i])) { 
          // 현재 단어 방문 표시
          visited[i] = true;
		  
          // 큐에 다음 단어 추가, 변환 횟수 1 증가
          queue.add(new Node(words[i], cur.change_cnt + 1));
        }
      }
    }
