...
class Solution {
  // 방향 벡터: 동, 남, 서, 북 (오른쪽, 아래, 왼쪽, 위쪽)
  int[][] wh = { {0, 1, 0, -1}, {1, 0, -1, 0} };

  public int solution(int[][] rectangle, int characterX, int characterY, int itemX, int itemY) {
    int answer = 0;

    // 좌표를 두 배로 확대하여 처리 (정밀도 향상)
    characterX *= 2;
    characterY *= 2;
    itemX *= 2;
    itemY *= 2;
    ...
    // 맵과 방문 여부를 나타내는 배열 초기화
    boolean[][] map = new boolean[102][102];
    boolean[][] vst = new boolean[102][102];

    // 사각형 내부를 true로 설정 (경로 표시)
