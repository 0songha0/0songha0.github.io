...
public class Solution {
  // 캐릭터에서 아이템까지의 최단 경로를 찾는 함수
  public int solution(int[][] rectangle, int characterX, int characterY, int itemX, int itemY) {
    // 이동 가능한 방향: 위, 아래, 왼쪽, 오른쪽
    final int[][] moves = { {0, 1}, {0, -1}, {-1, 0}, {1, 0} };
    
    // 방문한 위치를 저장하는 배열
    // 경계면으로 이동할 때 최단거리 정확도를 높이기 위해 좌표를 2배로 늘리기
    ...
    while (!queue.isEmpty()) {
      ...
      for (int i = 0; i < size; i++) {
        ...
        // 아이템 위치에 도달하면 이동 횟수 반환
        if (point[0] == itemX && point[1] == itemY)
          return answer;

        // 네 가지 방향으로 이동 시도
        for (int[] move : moves) {
          int newX = point[0] + move[0];
          int newY = point[1] + move[1];

          // 유효하지 않거나 이미 방문한 위치는 건너뜀
          if(newX < 0 || newY < 0 || visited[newX][newY]) continue;
          
          // 사각형의 경계에 없는 위치는 건너뜀
          if(!isOnRectangle(rectangle, point[0] + (double) move[0]/2,  point[1] + (double) move[1]/2)) continue;
          
          // 새로운 위치를 큐에 추가하고 방문한 것으로 표시
          queue.add(new int[]{newX, newY});
          visited[newX][newY] = true;
        }
      }
      
      // 현재 레벨의 모든 위치를 처리한 후 이동 횟수를 증가
      answer++;
    }

    // 큐를 다 소진해도 아이템을 찾지 못한 경우 -1 반환 (유효한 입력에서는 발생하지 않음)
    return -1;
  }

  // 주어진 점이 사각형의 경계에 있는지 확인하는 함수
