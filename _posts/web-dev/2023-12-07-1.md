...
<mark>기존 SSO 로그인 연동 로직 문제점</mark>  
...
결국 유저 쿠키 없이 유저 토큰만 받고도 사용자 정보를 응답하는 신규 SSO API를 생성 요청하여 받았습니다.
...
다른 팀 앱에서 로그인 후 우리 웹의 신규 컨트롤러 API 호출 시 암호화된 userkey를 보내주면,  
userkey를 복호화하고, 특정 문자열과 조합하여 암호화 후 신규 SSO API 1를 호출하여 유저 토큰을 받습니다.  
토큰을 파라미터로 신규 SSO API 2를 호출하면 암호화 된 유저 정보를 받을 수 있습니다.  
