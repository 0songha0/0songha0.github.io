...
첫 번째 API에 _sessid를 파라미터로 넘기고, 유효성 검증을 위한 유저 토큰과 SSO 쿠키를 생성 받습니다.  
...
iframe으로 쿠키 생성 API를 띄운 다음 로그인 유지 처리 ajax가 있는 화면으로 보내면 정상 동작하였습니다.
...
### AES256 암호화 함수
```
public static String authorizeKey(String userkeyDecode) {
...
<body class="main">
	<div style="display: none;">
		<iframe src="https://SSOAPI주소:446/api/auth/ValidationOutStudent?sessionData=${userkeyEncode}"></iframe>
	</div>
</body>
```
같은 URL이지만 ajax에서도 호출해서 유효성을 체크하고 화면을 분기해줘야 에러가 나지 않습니다.