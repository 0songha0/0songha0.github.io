...
<b>2. </b>컨트롤러에서 브라우저 세션 ID를 파라미터로 SSO Validation API를 호출하여 유효성 검증을 위한 Token을 발급 받습니다.  
<b>3. </b>Token을 파라미터로 SSO Authorize API를 호출하여 유효성 검증 후 사용자 정보 문자열을 반환 받습니다.  
<b>4. </b>사용자 정보 문자열 ssoData를 파싱해서 현재 사이트 세션 및 쿠키에 유저 정보를 저장하여 로그인 처리합니다.

<mark>기존 SSO 로그인 유지 로직 문제점</mark>  
안드로이드 삼성 인터넷 브라우저, iOS 사파리 등 모바일 기본 브라우저에서는 보안상 타 사이트 URL 쿠키가 차단되어,  
도메인이 다른 Cross site 쿠키 공유가 되지 않아 쿠키 유효성 검증을 통한 로그인이 되지 않습니다.

---

## 신규 SSO 로그인 기능 개발

<mark>신규 SSO 로그인 연동 로직 정리</mark>  
유저가 자체 로그인이 없는 우리 사이트에서 로그인 버튼을 누르면,  
우리 웹사이트 URL을 returnURL 파라미터로 다른 팀 웹 로그인 페이지를 호출합니다.  
해당 사이트에서 로그인 성공 시 우리 웹의 컨트롤러 API를 호출하여 암호화된 userkey를 보내주면,  
userkey를 복호화하고, 특정 문자열과 조합하여 암호화 후 신규 SSO API 1를 호출하여 유저 토큰을 받습니다.  
쿠키 없이 유저 토큰을 파라미터로 신규 SSO API 2를 호출하면 암호화 된 유저 정보를 받을 수 있습니다.  
유저 정보를 복호화해서 VO에 담고, 세션에 저장하여 세션 로그인 처리 후 메인 화면으로 이동합니다.  
로그인 실패 시에는 다시 다른 팀 웹 로그인 페이지로 보내주는 로직이 있어야 합니다.

returnURL 없이 타 사이트에서 로그인하고 들어온 경우 로그인 유지 처리는 별도로 해줘야 합니다.
