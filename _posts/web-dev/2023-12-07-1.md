...
해당 사이트에서 로그인 성공 시 우리 웹의 신규 컨트롤러 API를 호출하여 암호화된 userkey를 보내주면,  
...
유저 정보를 복호화 하면서 VO에 담고, 세션에 저장하여 세션 로그인 처리 후 메인 화면으로 이동합니다.  

returnURL 없이 타 사이트에서 로그인하고 들어온 경우의 로그인 유지 처리는 별도로 해줘야 합니다.
