...
```
iframe에서 호출하면 SSO 도메인에 쿠키가 정상적으로 생성됩니다.  
body 아래에서 noscript 태그로 감싼 이유는 모바일 안드로이드:삼성인터넷브라우저, iOS:사파리 브라우저 호환을 위해서입니다.  
같은 URL을 ajax에서도 호출해서 유효성을 체크하고 화면을 분기해줘야 에러가 나지 않습니다.  