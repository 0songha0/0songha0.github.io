...
## SSO 쿠키가 생성되지 않는 경우 SSO 로그인 방법

타 팀 앱에서 우리 웹으로 이동 시 SSO 로그인 유지되는 처리를 개발하고 있었습니다.  
첫 번째 API에 userkey를 넘기고, 두 번째 API에서 토큰과 쿠키를 넘기면 유저 정보를 응답해주기로 했습니다.  
첫 번째 API에서 쿠키를 생성해주었다는데 타 사이트 URL 쿠키라 우리 사이트에서 활용할 수 없었습니다.  
안드로이드:삼성 인터넷 브라우저, iOS:사파리 등 모바일 기본 브라우저에서는 보안상 타 사이트 쿠키가 차단되어,  
도메인이 다른 Cross site 쿠키 공유가 되지 않아 쿠키 확인을 통한 로그인이 되지 않기 때문입니다.  
결국 유저 쿠키 없이 유저 토큰만 받고도 사용자 정보를 응답하는 신규 SSO API를 생성 요청하여 받았고,  
두 번째 API 주소를 변경하여 해결하였습니다.
...
@RequestMapping(value = "/SSO로그인.do")
  ...
  try {
    ...
    // 메인 화면으로 이동 (로그인 성공 화면)
    return "redirect:/user/mmbrStndrd/selectListStu.do";

  } catch (Exception e) {
    LOGGER.debug("SSO Login Err userkey : " + userkey);
    LOGGER.debug("SSO Login Err : " + e.getMessage());
  }
		
  // 로그인 화면으로 이동 (로그인 실패 화면)
  return "view/user/cmmn/appLoginCmmnStu";
}
```

### Util 코드
```
public static LoginSso loginAppStu(String userkey) throws Exception {
  ...
  if (jsonObj.get("StatusCode") != null) {
    ...
    if ("AUTH_OK".equals(StatusCode)) {
      ...
      // 유저 데이터 복호화
      loginSso = loginProfSSOPRAM(ssoEncData);
...
### HttpURLConnection 객체 코드
<https://songha0.github.io/web-dev/2023-12-13-1>{: target="blank"}
