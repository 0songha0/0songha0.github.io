...
## 기존 Javascript에서 SSO 로그인 유지 처리 분석

### Interceptior에서 각 화면 로그인 여부 체크
HandlerInterceptorAdapter를 상속해서 구현한 AuthenticUserInterceptor의 preHandle 함수에서 로그인 여부와 권한을 체크하고 데이터를 담아줍니다.
...
<mark>AuthenticUserInterceptor에서 로그인 여부 체크 예시</mark>
```
