...
public class ImageUtil {
    ...
    public File makeResizeImg(File sourceFile, Integer width, Integer height) throws Exception {
        ...
        // 리사이징 시 화질이 어느정도 깨지는 것은 감안해야 한다.
...
```
위 유틸 소스로 JPG, PNG는 썸네일, 워터마크가 정상적으로 생성되는 것을 확인했습니다.  
...
```
이미지 리사이징 후 워터마크 이미지 생성 함수 호출 예시입니다.

---

## Unsupported Image Type 에러 해결

### ImageIO.read 시 에러 메세지
```
...
```
의존성 추가 후 Gradle 재빌드, 프로젝트 재시작만으로도 소스 변경 없이 위 에러를 해결해주는 라이브러리입니다.

<mark>imageio-jpeg 3.7.0 에러</mark>
```
