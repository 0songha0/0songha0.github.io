...
### class명을 가진 모든 요소에 ID 추가
```
const elements = document.querySelectorAll('.클래스명');

elements.forEach(function(element, index) {
    element.setAttribute('id', 'ID명_' + (index + 1));
});
```
NodeList를 반환하는 querySelectorAll는 forEach를 사용할 수 있고,  
HTMLCollection를 반환하는 getElementsByClassName은 forEach를 사용할 수 없습니다.
