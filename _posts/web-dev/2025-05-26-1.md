...
<table class="table_2_left">
  <tbody>
    <tr>
      <td>.stop</td>
      <td>겹쳐있는 요소들에 이벤트 전파를 막는 e.stopPropagation() 기능</td>
    </tr>
    <tr>
      <td>.prevent</td>
      <td>요소가 가진 기본 기능을 막는 e.preventDefault() 기능</td>
    </tr>
    <tr>
      <td>.capture</td>
      <td>
        이벤트 리스너를 캡처모드로 실행 가능<br><br>
        <strong>캡처링</strong> : 이벤트가 최상위 요소에서부터 현재 요소까지 내려가며 전파됨<br>
        <strong>버블링</strong> : 이벤트가 현재 요소에서부터 최상위 요소까지 올라가며 전파됨
      </td>
    </tr>
    <tr>
      <td>.self</td>
      <td>자기 자신만 호출 가능 (타깃 요소가 self 일 때 동작)</td>
    </tr>
    <tr>
      <td>.once</td>
      <td>이벤트를 한 번만 실행</td>
    ...
    <tr>
      <td>.passive</td>
      <td>모바일 장치 성능 개선을 위해 터치 이벤트 리스너와 함께 사용</td>
    </tr>
  </tbody>
</table>
