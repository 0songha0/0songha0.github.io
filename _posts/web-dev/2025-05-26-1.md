...
```
v-model 전달인자로 props명을 지정할 수 있습니다.  
하위 컴포넌트에서는 아래와 같이 받아서 사용할 수 있습니다.
```
<template>
...
// 스크립트에서도 emit 이벤트 발생 가능
const updateProps명1 = () => {
  emit('update:props명1', 변경값);
};
```
...
<template>
  <자식컴포넌트명>
    ...
    <template v-slot:default>본문 콘텐츠</template>
...
```
자식 컴포넌트 변수를 부모 컴포넌트에서 사용할 수 있도록 슬롯 콘텐츠에 전달합니다.  
