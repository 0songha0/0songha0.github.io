...
### 톰캣 추가
우측 상단 Add Configurations > 좌측 상단 + 버튼 > Tomcat Server : Local 선택 > Application server : Configuration 클릭 > 좌측 상단 + > Tomcat Home : 프로젝트 톰캣 폴더 선택 (예시 : apache-tomcat-9.0.82) > OK > Name : 톰캣명 버전 입력 > Store as project file 체크 (팀원에게 동일한 톰캣 설정 공유 가능. 팀원마다 톰캣 설치 경로가 다른 경우 체크X) > HTTP Port : 프로젝트 URL 포트 입력 > Before launch : Build '프로젝트명:war exploded' artifact (톰캣에 war 배포) 후 Build (최신 코드 컴파일) 순서로 배치 > Apply > OK
