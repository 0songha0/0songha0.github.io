...
## 스프링 필터, 인터셉터 공통점
필터, 인터셉터는 전역 처리에 주로 사용되고, 여러 개 등록 시 순서 제어가 가능합니다.
...
- 실행 시점 : DispatcherServlet 전달 전후 (doFilter)
- 요청 조작 : 가능 (요청 Request를 HttpServletRequestWrapper 등으로 감싸서 전달)
- 응답 조작 : 가능 (HttpServletResponseWrapper 사용)
- 필터 사용 목적 : 인코딩, 보안 필터링, 파라미터 변환, CORS, XSS 방어 등

### Spring Intercepter
- 소속 : Spring container (DispatcherServlet 내부)
- 실행 시점 : Controller 실행 전 (preHandle), Controller 실행 후/뷰 렌더링 전 (postHandle), 뷰 렌더링 후 (afterCompletion)
- 요청 조작 : 불가능 (Request 파라미터 변경 불가. Attribute 추가는 가능)
- 응답 조작: 뷰 모델(Model)에 데이터 추가 가능
