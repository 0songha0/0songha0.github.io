...
```
수정 화면 진입 시, URL에 포함된 ID 파라미터를 @PathVariable을 통해 받는 예시입니다.
...
@RequestBody는 @ModelAttribute와 달리 setter만 호출해서 바인딩하므로,  
getter 함수에서 원본 객체가 아닌 복사본을 반환하는 VO, DTO에도 매핑이 가능합니다.
...
DTO, VO 등에는 바인딩할 수 없고, 문자열, 숫자, 배열, Map, List 등 기본 타입에만 매핑 가능합니다.
...
VO, DTO는 getter 함수를 통해 원본 객체를 받아서 매핑하므로, 복사본을 반환하면 이용할 수 없습니다.
