...
module.exports = (io) => {
  ...
  io.on("connection", (socket) => {
    ...
    // 클라이언트에서 보낸 "message" 이벤트 수신
    socket.on("message", (msg) => {
      console.log("받은 메시지 : ", msg);

      // 모든 클라이언트에게 "message "이벤트 전송 (Broadcast)
      io.emit("message", msg);
    });

    // 클라이언트에서 보낸 "join" 이벤트 수신
...
// 소켓 클라이언트 인스턴스 생성
const socket = io("http://백엔드서버주소:포트", {
...
// 소켓 백엔드 서버 수동 연결
// 소켓 인스턴스 옵션 autoConnect: false인 경우
socket.connect();

// 소켓 서버에 이벤트 전송
socket.emit("이벤트명", 전송할데이터);
또는
socket.emit("이벤트명", 전송할데이터, (res) => {
  // 서버에서 acknowledgement로 응답한 경우 실행되는 콜백함수 작성
