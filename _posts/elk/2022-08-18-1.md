...
엘라스틱서치는 글로벌 오픈소스라 영어 문법에 특화된 standard 분석기가 기본 분석기로 적용되어 있습니다.  
standard 분석기는 한글을 단어가 아닌 공백으로만 분리하여 저장하기 때문에 한글 분석이 안 됩니다.  
한글 검색 정확도를 높이려면 반드시 한글 형태소 분석에 특화된 Nori 형태소 분석기 등을 따로 설치해서 적용해야 합니다.
...
형태소 분석기 설치 후 elasticsearch 재시작 시 적용됩니다.
...
생성된 인덱스의 analysis는 변경 불가하며, 변경 시 인덱스 삭제 후 재생성 또는 리인덱싱 필요합니다.
...
```
Nori 토크나이저에 제외단어, 제외품사 필터를 적용하였습니다.
...
단점은 숫자, 일부 영어 등이 검색되지 않을 수 있습니다.
...
```
커스텀 토크나이저에 "decompound_mode": "mixed" 설정을 주고 생성했기 때문에 위와 같은 결과가 나옵니다.
...
DFR, DFI, IB, LM Dirichlet, LM Jelinek Mercer 등 다른 알고리즘도 지원합니다.  
_settings을 변경하여 사용 가능합니다.
...
<mark>boost</mark>  
관련성이 높은 필드나 키워드에 더해주는 가중치입니다.  
fields 속성의 필드명 뒤에 ^2 등 붙여서 가중치 부여 가능합니다.

<mark>IDF</mark>  
인덱스의 전체 document 내 용어 등장 빈도 (역문서빈도) 입니다.  
낮을수록 연관성이 높습니다.
...
<mark>TF</mark>  
해당 document 내 용어 등장 빈도 (용어빈도) 입니다.  
높을수록 연관성이 높습니다.
