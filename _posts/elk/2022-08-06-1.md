---
title: "Logstash로 MariaDB 데이터 수집 후 엘라스틱서치 저장 방법"
excerpt: ""
...
```
DB 연결이 가능한 SpringBoot 프로젝트에서 Build.gradle에 추가하고 Gradle 빌드 후 2.7.3 버전 jar를 얻었습니다.
...
<mark>/usr/share/logstash/tools 폴더에 업로드 시 에러메세지</mark>
```
open for write: permission denied
```
해당 폴더의 권한을 변경하거나, /home/유저명 폴더에 올린 후 아래 명령어로 이동시켜주면 됩니다.
```
...
- 1. Management 메뉴 > Data > Index Management
logstash로 수집 시 자동 생성된 index명을 확인합니다.

- 2. Management 메뉴 > Kibana > Data Views > 우측 Create data view 버튼
Name : index명* 으로 Data Views를 생성합니다.

- 3. Analytics 메뉴 > Discover
...
여러 로그스태시 수집 conf 파일을 한번에 실행하는 방법입니다.
