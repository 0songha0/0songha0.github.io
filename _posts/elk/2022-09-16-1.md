---
title: "JAVA에서 엘라스틱서치 REST API 호출 및 사용 방법"
excerpt: ""
...
```
elasticUtil을 사용하여 엘라스틱서치 데이터를 검색하는 ElasticQuery 클래스입니다.  
Mapper처럼 쿼리를 따로 모아두면 관리가 편하며, 재사용성을 높일 수 있습니다.

Json 문자열은 JSONObject를 이용해서 만들어도 되지만,  
Query DSL은 뎁스가 많아 복잡해지기 때문에 가독성과 편의를 위해 String으로 만들었습니다.

위 String += 방법은 성능이 좋지 않아서, 아래처럼 StringBuilder를 사용하여 메모리 부하를 줄여주는 것이 좋습니다.
