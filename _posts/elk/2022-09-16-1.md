...
포스트맨으로 엘라스틱서치 Rest API 호출이 가능하니 시도해 볼까 했으나,  
...
스프링부트에서 JAVA 엘라스틱서치 유틸 생성 후 호출하는 형식으로 구현해보았습니다.
...
return resultMap;
```
Json은 JSONObject로 만들어도 되지만, Query DSL은 뎁스가 많아 복잡해질 수 있어 가독성과 편의를 위해 String으로 만들었습니다.

String +=는 성능이 안좋으니, 아래처럼 StringBuilder를 사용하여 메모리 부하를 줄여주면 더 좋습니다.
```
Map<String, Object> resultMap;
...
1. Notepad++에 키바나 Console에서 완성한 Query DSL을 붙여 넣습니다.
2. "를 \"으로 모두 바꾸기 합니다.
3. 한 줄마다 " ";로 감싸줍니다.
