...
<mark>CODE-RAY 취약점 보완 팁</mark>  
취약점 상세 화면 우측의 연관 라인 최하위 코드 선택 후 IDE에서 검색하고 수정하는 것이 좋습니다.  
한 파일에서 여러 취약점이 나온 경우, CODE-RAY 툴 내에서 수정 저장하면 반영이 안됩니다.
...
```
아래와 같이, 새로운 리스트 객체를 생성하고 값을 깊은 복사하면 데이터 변경 시 원본에 영향을 주지 않아 안전합니다.
```
...
```
위 방식은 원본 리스트에 데이터를 세팅할 수 없으므로 @ModelAttribute로 데이터를 받을 수 없습니다.
...
### 중요정보 평문저장
```
System.out.println("classProfSearch ehCache Put : " + cacheKey);
```
위와 같이, 중요한 정보를 출력하는 코드도 제거 대상입니다.
