...
### NCP 서버 이벤트 확인
NCP Console 좌측 Server 메뉴 > Event 메뉴  
각 서버 인스턴스에 자동 정지 및 시작 이벤트가 수행된 시각을 알 수 있습니다.  
수행예정 시각과 별개로, 수행완료 시각이 실제 완료 시간입니다.

### NCP SourceCommit Git 생성
NCP Concole 좌측 Services 메뉴 > Commit 검색 > SourceCommit 선택 > 리포지토리 생성 버튼 > 리포지토리 이름 : 프로젝트명 작성, 리포지토리 설명 : 프로젝트 설명 작성, 초기화 설정 Add a READEME 체크 해제, Add a .gitignore: None 선택 > 다음 > 다음 > 생성

### NCP 서브계정 생성
NCP 메인계정 또는 모든 기능을 이용할 수 있는 NCP_ADMINISTRATOR 권한이 부여된 서브계정 로그인 > 우측 상단 콘솔 > NCP Concole 좌측 Services 메뉴 > Account 검색 > Sub Account 선택 > 리소스 : 서브 계정 선택 > 서브 계정 생성 > 로그인 아이디, 사용자명, 이메일 입력 > 접근 권한 : 콘솔 접근 (모든 곳에서 접근 가능), API Gateway 접근 (모든 Source에서 접근 가능) 선택 > 로그인 비밀번호 : 자동 생성 > 생성 > 자동 생성된 비밀번호 메모

<mark>NCP 서브계정 SourceCommit Git 사용 권한 부여 ★</mark>  
NCP 메인계정 또는 모든 기능을 이용할 수 있는 NCP_ADMINISTRATOR 권한이 부여된 서브계정 로그인 > 우측 상단 콘솔 > NCP Concole 좌측 Services 메뉴 > Account 검색 > Sub Account 선택 > 서브 계정 선택 > 개별 권한 추가 > 정책명 : NCP_SOURCECOMMIT_MANAGER 검색 > 관리형 정책 : NCP_SOURCECOMMIT_MANAGER (SourceCommit 서비스 내 모든 기능을 이용할 수 있는 권한) 선택 > 추가  
해당 권할을 부여하지 않으면 SourceCommit 서비스가 보이지 않습니다.

### NCP SourceCommit Git 계정 생성
NCP 서브계정 로그인 > 초기 로그인 시 비밀번호 변경, 휴대폰 2차 인증 등록 > 우측 상단 콘솔 > NCP Concole 좌측 Services 메뉴 > Commit 검색 > SourceCommit 선택 > GIT 계정/GIT SSH 설정 버튼 > User name : 로그인 되어있는 서브 계정 이름으로 고정됨, Password : 신규 입력 > 적용  
NCP 서브계정 1개당 Git 계정 1개만 생성할 수 있습니다.
...
기존 운영 DB는 매일 0시 5분에 백업되도록 설정되어 있고, 50GB 백업에 7분 소요되었습니다.
...
증분 백업은 복구 시 전체 백업 + 중간 증분 백업들을 순서대로 적용하기 때문에,  
마지막 전체 백업 이후에 변경된 데이터만 백업하는 차등 백업보다 복구 시간이 오래걸릴 수 있습니다.

### NCP DB 백업 파일 저장 방법
NCP Console 좌측 Services 메뉴 > Cloud DB for MySQL > Bakup 메뉴 > 상세내역 > 백업 파일 선택 > Object Storage로 보내기 > Bucket 및 Folder 선택 > Object Storage로 보내기  
SaaS (CDB Mysql) 상품을 이용하는 경우, 해당 위치에 날짜별 백업 파일들이 저장됩니다.  
Object Storage 이용 신청 후 버킷을 생성해야 내보낼 수 있고, 50GB 백업 파일은 17분 정도 소요됩니다.  
다운 시에는 '2GB초과 object는 api를 이용하여 다운로드 해 주세요.'라는 팝업이 나옵니다.  
.sql이 아닌 Xtrabackup 계열 물리 백업 파일이므로, 같은 NCP DB에 복원할 때 이용하는 것이 좋습니다.  
.sql 파일은 DBeaver 또는 MySQL 터미널 등에서 생성 가능합니다.
