...
<mark>특정 호스트에서 연결 불가 에러메시지</mark>  
```
Host '10.10.1.87' is not allowed to connect to this MySQL server
```
위와 같은 오류가 나오는 경우, MySQL 터미널에 접속해서 아래 명령어를 통해  
모든 호스트에서 접근 가능한 유저 생성 후 다시 시도하면 됩니다.
```
CREATE USER '유저명'@'%' IDENTIFIED BY '비밀번호';
GRANT ALL PRIVILEGES ON *.* TO '유저명'@'%';
FLUSH PRIVILEGES;
```
%는 특정 IP가 아니라 모든 IP 접속을 허용합니다.

<mark>공개키 가져오기 불가 에러메시지</mark>
```
Public Key Retrieval is not allowed
```
MySQL 8 인증 방식 때문에 DBeaver/JDBC에서 공개키 가져오기를 막아서 생긴 오류입니다.  
Driver properties 탭에서 아래 속성 값들을 변경하면 연결됩니다.
```
allowPublicKeyRetrieval = true
useSSL = false
```
만약 개발 DB라면, 해당 호스트 계정에 대해서 최신 인증 방식 caching_sha2_password가 아니라  
구버전 MySQL 인증 방식 mysql_native_password 로 변경해도 연결이 됩니다.
...
```
위와 같이, 상단 [mysqld] 텍스트 아랫줄에 해당 옵션을 추가하면 됩니다.  
