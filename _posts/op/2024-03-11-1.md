...
- WAS 서버 앞에 WEB 서버가 있는 경우 : 웹사이트 루트 디렉터리 (apache WEB 서버의 경우, httpd.conf 파일에 설정된 DocumentRoot)에 robots.txt 파일을 만들고 크롤러명, 접근하면 안 되는 URL (/)을 명시하면 됩니다. 웹사이트URL/robots.txt에 해당 파일이 위치해야 하며, 웹 서버를 재기동할 필요는 없습니다.
