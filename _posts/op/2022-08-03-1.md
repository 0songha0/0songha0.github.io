...
로드밸런싱 해주는 MVP 서버 IP로 curl 날렸을 때 간헐적으로 성공/실패(타임아웃) 하는 경우,  
로드밸런싱 되는 대상 서버 IP 또는 도메인으로 직접 curl 날렸을 때, 연결이 된다면 서버 내부 방화벽은 허용된 것입니다.  
MVP 서버에서 외부 통신을 받을 수 있도록 인프라팀에서 설정을 해주셔야 정상적으로 통신이 가능했습니다.  
(MVP 서버 아웃바운드 방화벽 설정을 변경하여 통신이 원활하도록 하용해주신 것 같습니다.)
...
API 요청을 받는 MVP 서버 도메인에 curl 명령어 시 간헐적으로 성공/실패(타임아웃) 하는 경우,  
MVP 서버의 로드밸런싱 대상 Real 서버 IP에는 연결이 정상적으로 된다면  
현재 서버와 대상 서버 방화벽은 정상적으로 열려있으나, MVP 아웃바운드 방화벽이 닫혀 있을 수 있습니다.
