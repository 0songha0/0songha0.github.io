...
1) 소스 유실 방지를 위해 현재 작업중 소스 Stash  
2) Fetch 해서 원격 저장소 브랜치들의 Push 이력을 가져옵니다.  
3) develop 브랜치에 변경 내용이 있다면, develop 브랜치 우클릭 > pull 또는 update  
4) 개인브랜치로 이동하여 develop 브랜치 우클릭 > Merge 'develop' into '개인브랜치' > Merge 이력 Push  
5) 작업한 소스를 Unstash로 되살리고 Commit and Push  
6) 다시 develop 브랜치로 이동하여 Merge '개인브랜치' into 'develop' > Merge 이력 Push
