...
좌측 하단 Git > Log > Remote > branch명 우클릭 > Checkout  
해당 branch와 같은 이름의 Local branch가 생성되며, 최신 소스로 내려받아집니다.

### Git Local branch 복제
Local > main branch명 우클릭 > New branch from 'branch명' > New branch name : 신규 branch명 입력 > Create
...
### Git push 절차
<mark>Pull</mark>
push 하고 싶은 branch로 이동 > Git 좌측 아이콘 Fatch All Remotes > update Selected

<mark>commit</mark>
프로젝트 우클릭 > Git > Commit Directory > Commit 할 파일들 선택, Commit Message 작성 후 Commit > ↗ 아이콘이 생겨있는 Git Local branch 우클릭 > push  
Git history 눌러서 파일 비교해보면 왼쪽이 이전 소스, 오른쪽이 변경된 소스입니다.

### Commit 제외 폴더 설정
.gitignore 파일에 명시하면 됩니다.

### 새로운 Git 저장소에 첫 소스 올리기
Intellij에서 프로젝트 만들고 상단 VCS > Enable Version Control Integration... > 좌측 하단 Git 보면 빨간 Unversioned Files 파일들이 있는데 전부 체크 > commit > Commit Message 작성 > commit > 상단 Git > push > Define remote > 
<table>
<tbody>
  <tr>
    <td>Name</td>
  ...
  <tr>
    <td>URL</td>
    <td>https://git-codecommit.ap-northeast-2.amazonaws.com/v1/repos/프로젝트명</td>
  </tr>
</tbody>
</table>
...
### Unversioned files (빨간색) 커밋하는 법
프로젝트명 또는 커밋하려는 파일 우클릭 > Add 후 Changes 폴더에 들어가면 commit 가능합니다.

### Git 원격리포지토리 히스토리가 없어 pull 받을 수 없는 경우
Git 원격리포지토리 연결이 끊긴 상태에서 Commit 했기 때문에 다시 Git 원격리포지토리 연결하고 fetch한 후에도 로컬리포지토리에 원격리포지토리 히스토리가 안생기고, 새로 Commit한 히스토리만 있는 상태라서 Pull이 거부되는 상황입니다.  
git bash로 프로젝트 경로에서 git stash 명령어로 현재 소스 임시저장 후 Pull하니 Merge branch 히스토리가 남고, 로컬리포지토리에 이전 원격 히스토리들도 정상적으로 생겼습니다.

### 이전 커밋 소스로 돌아가는 법
로컬리포지토리 히스토리 리스트에서 돌아가고 싶은 Commit 시점의 히스토리에 우클릭 > Checkout Revision 'commit id (hash 값)' 시 노란 연필 아이콘이 해당 히스토리로 이동하며 로컬 소스도 해당 커밋 이후의 소스로 변경됩니다.

<mark>Git 아이콘 구분</mark>
<table>
  <tbody>
    <tr>
      <td>노란 연필 아이콘</td>
...
Rollback 후 commit 시 HEAD가 꼬이는 경우가 많아 잘 사용하지 않습니다.

<mark>reset<mark>
해당 커밋시점 직후로 커밋 기록을 되돌립니다.  
IntelliJ에서 Reset은 충돌 확률이 높기 때문에, Revert 사용을 권장합니다.

<mark>Revert<mark>
취소하려는 커밋 우클릭 > Revert Commit 시 Revert "커밋명" 시 그 커밋의 취소 커밋이 하나 더 생깁니다.
