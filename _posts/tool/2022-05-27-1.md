...
### IntelliJ 한글 언어팩 설정
File > Settings... > Plugins > Installed > Korean 검색 > Korean Language Pack / 한국어 언어 팩 체크 > OK > 재시작  
한국어 언어 팩 플러그인 체크를 해제하고 재시작하면 다시 영문으로 돌아옵니다.

### jsp 소스 변경 시 톰캣 재시작 없이 적용되게 설정
우측 상단 Edit Configurations > Tomcat Server > Server 탭 > On frame deactivation : Update classes and resources  
IntelliJ 창을 벗어날 때마다 Java 클래스, JSP 등 리소스를 톰캣에 재배포합니다.

On 'Update' adcion는 IntelliJ에서 직접 Update 버튼을 눌렀을 때 반영하는 설정이며,  
톰캣을 재시작하는 기본값 Restart server는 효율적이지 않아 Update classes and resources가 권장됩니다.
...
### 현재 동작 이름 및 단축키 표시 설정
File > Settings > Appearance & Behavior > Presentation Assistant  
실행한 동작마다 이름 및 단축키를 화면에 잠시 표시합니다.
