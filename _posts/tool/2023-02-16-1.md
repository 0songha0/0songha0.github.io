...
MySQL 8.0.32 형태의 DB 버전을 확인할 수 있습니다.
...
EXPLAIN SQL문
```
쿼리 앞에 explain을 붙이고 실행하면 아래 Results에 쿼리의 예상 실행계획이 그리드 형태로 나옵니다.  
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        <b>const ★</b> : 기본키 또는 유니크 인덱스를 사용하여 상수처럼 취급되는 한 행 스캔 (효율적)<br>
        <b>eq_ref</b> : 조인에서 기본키 또는 유니크 인덱스를 사용하여 한 행 스캔<br>
        <b>ref ★</b> : 비유니크 인덱스 (유니크 제약조건이 걸려있지 않은 컬럼의 인덱스) 또는 유니크 인덱스의 일부 컬럼을 사용하여 특정 값과 일치하는 한 행 또는 여러 행 스캔<br>
    ...
    <tr>
      ...
      <td>
        ...
        100에 가까울수록 필요한 행만 가져온 것이므로 성능이 좋습니다.
      </td>
    </tr>
    <tr>
      <td>Extra</td>
...
```
쿼리를 실제로 실행하면서 수집한 실행 통계 기반 실행계획을 계층형 문자열로 보여줍니다.  
하위 노드부터 상위 노드로 올라오면서 확인하는 것이 좋습니다.

<mark>EXPLAIN ANALYZE 실행계획 해석 예시</mark>
```
-> Filter: (테이블명.컬럼명 = 조회조건) (cost=0.95 rows=1) (actual time=0.0646..0.0748 rows=2 loops=1)
    -> Table scan on 테이블명 (cost=0.95 rows=7) (actual time=0.0563..0.0673 rows=7 loops=1)
```
실제 실행계획에서 각 요소는 아래와 같이 해석할 수 있습니다.
<table class="table_2_left">
  <tbody>
    <tr>
      <td>Filter:</td>
      <td>필터링 작업 (불필요한 row를 제거하는 후처리)</td>
    </tr>
    <tr>
      <td>Table scan on 테이블명</td>
      <td>테이블 풀스캔 작업</td>
    </tr>
    <tr>
      <td>Index range scan on 테이블명 using 인덱스명</td>
      <td>인덱스를 사용한 범위 스캔 작업</td>
    </tr>
    <tr>
      <td>Index lookup on 테이블명 using 인덱스명</td>
      <td>인덱스를 사용해 조건에 맞는 row 건별 탐색 작업</td>
    </tr>
    <tr>
      <td>actual time=0.0646..0.0748</td>
      <td>
        현재 작업의 실제 수행 시간<br/>
        하위 노드 작업 시간도 누적되어 있습니다.<br/><br/>
        0.0646 = 첫 번째 row를 반환하기까지 걸린 시간 (ms)<br/>
        0.0748 = 마지막 row까지 처리하는데 걸린 시간 (ms)
      </td>
    </tr>
    <tr>
      <td>rows=1e+6</td>
      <td>
        현재 작업 노드가 실제로 반환한 row 수<br/><br/>
        1e+6 = 10⁶ = 1,000,000 = 100만
      </td>
    </tr>
  </tbody>
</table>
...
## 조회 limit 변경 방법
조회 결과 하단 톱니바퀴 우측 인풋 값 수정  
DBeaver에서 조회 쿼리 실행 시 결과 조회 행 수 제한을 설정할 수 있습니다.
...
<mark>개발 DB 운영 대상 데이터 조회 쿼리 예시</mark>
```
SELECT * FROM 개발테이블명 a
WHERE DEL_YN = 'N'
AND 기타 조건 체크
AND 키컬럼 in (
'MMBR0000000000001',
...
AND NOT EXISTS ( -- 운영에 이미 있는 데이터 중복 제거 이관
  SELECT 1
  FROM 운영복제테이블 b
  WHERE b.키컬럼1 = a.키컬럼1
    AND b.키컬럼2 = a.키컬럼2
...
```
이관하려는 각 테이블마다 운영 이관 대상 데이터 SELECT 쿼리를 만들어야 합니다.

---

## DB 마이그레이션 방법

### DB 전체 .sql 백업 파일 생성 방법
DBeaver에서 백업 대상 DB 서버 클릭 > Databases > DB 우클릭 > 도구 > Dump database > 다음 > Output Folder 지정 > Start > 완료 메시지 확인 후 취소 버튼으로 닫기   
운영 DB Dump는 사용자가 적은 시간대에 실행하는 것이 좋습니다.  
백업 파일 크기가 28GB 정도라면, 1시간 17분 정도 소요될 수 있습니다.

<mark>Dump 정상 완료 메시지</mark>
```
C:\Users\DB400TDA\AppData\Roaming\DBeaverData\drivers\clients\mysql\win\mysqldump.exe --skip-lock-tables --routines --add-drop-table --disable-keys --extended-insert -u 계정명 --host=DB서버도메인 --port=3306 DB명
Task 'MySQL dump' started at Wed May 06 15:16:41 KST 2026
Task 'MySQL dump' finished at Wed May 06 16:23:02 KST 2026
```
위와 같이, 정상 finished 되면 팝업을 닫아도 됩니다.
...
DB 복원 중 반응이 없는 게 정상이며, DBeaver로 접속 시 테이블이 생기고 있는지 확인할 수 있습니다.

28GB 운영 DB Dump 파일을 스테이징 DB에 복원 시 1시간 12분 정도 소요되었습니다.

.sql 파일 실행 중 에러가 발생하면 이후 작업은 중단됩니다.

<mark>DB Dump 파일 앞부분 확인 명령어</mark>
```
head -n 50 파일명.sql
```
CREATE DATABASE 구문이 있는 경우, 직접 DB를 생성하지 않아도 자동 생성 됩니다.  
DB Dump 파일 앞부분에 CREATE DATABASE 구문이 없으면 기존 DB DDL을 우선 실행해야 합니다.
