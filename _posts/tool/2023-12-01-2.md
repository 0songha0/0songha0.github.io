...
### 윈도우 Git 설치 방법
<https://git-scm.com/install/windows>{: target="_blank"}  
윈도우 cmd에서 git 명령어를 사용하려면 git 설치가 필요합니다.
...
### Git 이관 방법
AWS CodeCommit Git을 복제하여 NCP SourceCommit Git에 push 하였습니다.
```
mkgit gitMigration
cd gitMigration
```
Git 마이그레이션 전용 임시 폴더를 생성합니다.
```
git clone --mirror https://git-codecommit.ap-northeast-2.amazonaws.com/v1/repos/리포지토리명
cd 리포지토리명.git
```
기존 AWS CodeCommit Git의 모든 브랜치, 태그, refs 등을 완전 복제합니다.
```
git remote add ncp https://devtools.gov-ncloud.com/1234/리포지포리명.git
git push --mirror ncp
```
복제된 Git 폴더에 NCP SourceCommit Git 리포지토리를 ncp라는 별칭의 새 원격 저장소로 추가합니다.  
추가된 NCP Git 원격 저장소에 기존 프로젝트 코드 및 히스토리를 복제하여 Push 합니다.  
push 시 나오는 Git Credential Manager 팝업에 신규 NCP Git 사용자명 및 암호 입력이 필요합니다.
```
cd workspace
git clone https://devtools.gov-ncloud.com/1234/리포지포리명.git
```
기존 작업 폴더에 이관한 NCP Git 프로젝트 코드를 내려받습니다.
