...
```
도커 컨테이너를 삭제하여 저장된 모든 데이터를 제거할 수 있습니다.

### 도커 컨테이너 내부 쉘 접속
```
docker exec -it 컨테이너명또는ID bash또는sh
```
윈도우 docker desktop 앱 Containers 메뉴에서 Container ID 확인이 가능합니다.

### 카프카 토픽 목록 조회
```
kafka-topics.sh --bootstrap-server localhost:9092 --list
```
cmd에서 도커 컨테이너 접속 후 카프카 CLI 도구로 저장된 토픽 등을 확인할 수 있습니다.
