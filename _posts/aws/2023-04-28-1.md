...
```
MySQL 8.0은 이미 DB 데이터가 있는 경우, lower_case_table_names를 1로 설정할 수 없습니다.  
기존 프로젝트 DB 테이블명이 모두 소문자이고 쿼리에서는 테이블명이 모두 대문자라서 다른 버전으로 재설치해서 다시 구축하였습니다.

운영 DB, 개발 DB 모두 같은 버전을 사용해야 쿼리 호환 관련 오류를 방지할 수 있습니다.  
초기 구축 단계에서 버전을 맞추고, lower_case_table_names 설정을 꼭 해두는 것이 좋습니다.

<mark>MySQL 8.0에서 lower_case_table_names 설정 방법</mark>  
기존 데이터 백업 후 삭제 (데이터가 있으면 my.cnf 파일에 대소문자 구분 설정 변경 시 DB 실행 오류 발생) > DB 종료 > /var/lib/mysql 폴더 제거 > my.cnf 파일에 lower_case_table_names=1 설정 > DB 접속 계정 설정 > DB 실행
