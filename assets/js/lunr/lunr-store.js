var store = [{
        ...
        "excerpt":"크롬 개발자 도구 사용법 크롬 개발자 도구 실행 방법 F12 (단축키) 크롬 우측 상단 점 3개 &gt; 도구 더보기 &gt; 개발자 도구 크롬 브라우저 우클릭 &gt; 검사 개발자 도구 위치 변경 F12 &gt; 우측 상단 점 3개 &gt; Dock side: 팝업창/좌측/하단/우측 중 선택 개발자 도구 언어 변경 F12 &gt; 우측...","categories": ["it-tip"],
        "tags": [],
        ...
        "title": "리눅스 폴더 및 파일 관련 명령어",
        "excerpt":"폴더 및 파일 관련 명령어 파일 편집 명령어 https://0songha0.github.io/op/2022-08-23-1 파일 압축 관련 명령어 https://0songha0.github.io/op/2024-03-07-1 전체 파일에서 문자열 검색 grep -rn 검색어 * 현재 폴더 아래의 모든 파일들 내에서 텍스트를 검색합니다. *를 파일명으로 바꾸면 한 파일 내에서 텍스트 검색도 가능합니다. -n 옵션으로 라인 번호도 함께 출력합니다. 특정 확장자 파일에서 문자열 검색...","categories": ["op"],
        "tags": [],
        "url": "/op/2022-08-16-1",
        "teaser": null
      },{
        ...
        "excerpt":"Git Clone이나 간단한 커밋 시에는 cmd에서 명령어로 하는 것이 빠를 수 있습니다. 소스 비교나 체리픽은 소스트리, IntelliJ 같은 GUI 툴 사용이 훨씬 편합니다. 윈도우 cmd에서 Git 사용법 윈도우 Git 설치 방법 https://git-scm.com/install/windows 윈도우 cmd에서 git 명령어를 사용하려면 git 설치가 필요합니다. Git 원격 브랜치 소스 가져오기 Default 브랜치 복제 git clone...","categories": ["tool"],
        "tags": [],
        ...
        "excerpt":"다크모드 설정 방법 윈도우 다크모드 설정 Windows 설정 &gt; 개인 설정 &gt; 색 &gt; 모드 선택 : 다크 선택 윈도우에서 관리하는 모든 웹 브라우저, 파일탐색기, 제어판 등에 다크모드가 적용됩니다. 크롬 다크모드 설정 크롬 우측 상단 점 3개 &gt; 설정 &gt; 모양 &gt; 모드 &gt; 어둡게 선택 크롬 상단바, 어두워져도 괜찮은...","categories": ["tool"],
        "tags": [],
        ...
        "excerpt":"단방향 해시 복호화가 불가능하여 비밀번호 해싱에 적합합니다. 해시 함수 MD5 128비트 고정 길이의 출력값을 가집니다. 보안이 취약해서 인증용으로 사용이 권장되지 않습니다. SHA-256 데이터 무결성 검증 등에 사용되는 해시 함수입니다. 속도가 빠르기 때문에 GPU 공격에 취약하여 비밀번호 저장 용도로는 부적합합니다. 사용자가 입력한 비밀번호를 해싱 후 SHA-256 해시 앞 16자리를 잘라서 비밀번호...","categories": ["web-dev"],
        "tags": [],
        ...
        "excerpt":"레이아웃 Tiles 설정 방법 egov-com-servlet.xml 예시 &lt;bean id=\"tilesViewResolver\" class=\"org.springframework.web.servlet.view.UrlBasedViewResolver\"&gt; &lt;property name=\"viewClass\" value=\"org.springframework.web.servlet.view.tiles3.TilesView\" /&gt; &lt;property name=\"order\" value=\"1\" /&gt; &lt;/bean&gt; &lt;!-- Tiles Configurer --&gt; &lt;bean id=\"tilesConfigurer\" class=\"org.springframework.web.servlet.view.tiles3.TilesConfigurer\"&gt; &lt;property name=\"definitions\"&gt; &lt;list&gt; &lt;value&gt;/WEB-INF/config/egovframework/tiles/common-def.xml&lt;/value&gt; &lt;/list&gt; &lt;/property&gt; &lt;/bean&gt; common-def.xml 파일을 definitions 프로퍼티로 사용하는 tilesConfigurer를 정의합니다. common-def.xml 예시 &lt;tiles-definitions&gt; &lt;definition name=\"baseLayout\" template=\"/WEB-INF/views/layouts/layout.jsp\"&gt; &lt;put-attribute name=\"header\" value=\"/WEB-INF/views/common/header.jsp\" /&gt; &lt;put-attribute...","categories": ["web-dev"],
        "tags": [],
        ...
        "excerpt":"Java 자료구조 Java 데이터 타입 종류 프리미티브 타입 (기본 데이터 타입) 메모리에 값을 직접 저장해서 연산 속도가 빠릅니다. 예시 : int, long, float, double, char 등 레퍼런스 타입 (참조 데이터 타입) 메모리에 값이 들어있는 객체의 주소를 저장합니다. 컬렉션 프레임워크 클래스의 제네릭에는 프리미티브 타입이 아닌 레퍼런스 타입 객체를 사용해야 합니다. 예시...","categories": ["python-java"],
        "tags": [],
        ...
        "excerpt":"여러 기업 채용공고에 자주 보이는 기술스택 개념을 익히기 위해 정리한 글입니다. DevOps 기술 Docker 컨테이너화된 애플리케이션을 격리된 환경에서 실행할 수 있는 오픈 소스 컨테이너 플랫폼입니다. 도커 내부적으로 containerd라는 컨테이너 런타임이 사용됩니다. 도커는 이미지라는 템플릿을 사용하여 컨테이너를 생성하고, 이를 저장하고 공유할 수 있는 레지스트리를 제공합니다. 도커 허브(Docker Hub)는 가장 잘 알려진...","categories": ["it-tip"],
        "tags": [],
        ...
        "excerpt":"클라우드 서비스 모델 종류 기존 온프레미스 애플리케이션, 데이터, 런타임, 미들웨어, 운영체제, 가상화, 서버, 스토리지, 네트워크 모두 직접 구축 및 관리합니다. 예시 : 기업 자체 데이터센터, 직접 운영하는 서버 IaaS (Infrastructure as a Service) 인프라 (가상화, 서버, 스토리지, 네트워크) 만 클라우드 서비스 제공 업체에서 관리합니다. 애플리케이션, 데이터, 런타임, 미들웨어, 운영체제는 직접...","categories": ["op"],
        "tags": [],
        "url": "/op/2025-04-30-1",
        "teaser": null
      },{
        ...
        "excerpt":"컴포저블 함수 (Composable) 반응형 상태 관리 로직을 캡슐화하여, 여러 컴포넌트에서 재사용하기 위한 함수형 모듈입니다. axios로 서버 API 호출하는 로직도 컴포저블 함수로 만들 수 있습니다. 컴포저블 함수 정의 방법 import { ref } from 'vue'; // 상태 변수는 모듈 범위에 정의하거나, 상태 관리 스토어 Pinia 사용 필요 // 모듈 범위에 상태...","categories": ["web-dev"],
        "tags": [],
        ...
        "excerpt":"스프링부트 프로젝트는 내장 톰캣이 있으므로, 톰캣을 따로 설치하지 않고 java로 실행할 수 있습니다. 리눅스 Java 설치 리눅스 JDK 압축 파일 다운 https://www.oracle.com/java/technologies/downloads/archive 오라클 로그인 후, 위 경로 우측에서 프로젝트 JDK 버전을 선택합니다. jdk-17.0.15_linux-x64_bin.tar.gz 같은 압축 파일로 받으면 설치가 간편합니다. rpm, deb 같은 패키지 설치 방식을 이용하는 것도 가능합니다. JDK 설치...","categories": ["op"],
        "tags": [],
        "url": "/op/2025-08-19-1",
        "teaser": null
      },{
        "title": "리눅스 Nginx 서버 구축 및 Vue 프로젝트 빌드 방법",
        "excerpt":"리눅스 Nginx 설치 시 참고 https://nginx.org/en/linux_packages.html Nginx 공식 홈페이지에서 리눅스 OS 종류별 Nginx 웹서버 설치 방법을 자세히 알려주고 있습니다. 우분투 Nginx 서버 설치 방법 (인터넷O) 인터넷이 가능한 서버에서만 가능한 방법입니다. Nginx 저장소 필수 패키지 설치 sudo apt install curl gnupg ca-certificates lsb-release ubuntu-keyring nginx 저장소 추가 시 필요한 필수 패키지들을...","categories": ["op"],
        "tags": [],
        "url": "/op/2025-08-21-1",
        "teaser": null
      },{
        ...
        "excerpt":"WebRTC 서버 아키텍처 WebRTC 서버 구성요소 SFU 클라이언트가 보낸 영상/음성 등 미디어 스트림을 받아서, 각 스트림을 필요로 하는 구독자 클라이언트들에게 선택적으로 전달하는 중계 서버입니다. CPU/메모리/네트워크 대역폭 등 리소스를 많이 사용하므로, 인스턴스 유형은 r6g.large 이상이 권장됩니다. 시그널링 서버 WebRTC에서 클라이언트 간 연결을 준비하는 서버입니다. SDP 정보 (미디어 종류, 코텍 정보 등),...","categories": ["op"],
        "tags": [],
        "url": "/op/2025-09-04-1",
        "teaser": null
      },{
        ...
        "title": "자주 쓰는 Vue 문법 / 유용한 Vue 기능 사용법",
        ...
        "title": "Java 객체지향 설계 시 사용되는 디자인 패턴 종류 및 예시 코드",
        "excerpt":"디자인 패턴 학습 시 UML 다이어그램은 클래스 관계, 상속, 의존성 등 정적인 구조를 표현하지만, 객체의 동적 흐름 표현이 부족해서 객체지향 패턴의 본질을 보여주기 어렵다고 합니다. 이 글에서는 Java 예제 코드를 이용하여 각 패턴의 의도와 동작 원리를 학습할 수 있습니다. 생성 패턴 싱글톤 패턴 (Singleton) 프로그램에서 특정 클래스의 인스턴스가 단 하나만...","categories": ["plan-design-db"],
        "tags": [],
        ...
        "excerpt":"Git 블로그 빌드 실패 Bundler, RubyGems 버전 불일치 에러 build-deploy The process '/opt/hostedtoolcache/Ruby/3.1.7/x64/bin/bundle' failed with exit code 5 위와 같은 메세지가 나오는 경우, 빌드에 사용되는 Bundler와 루비 버전이 호환되지 않아 실패했을 수 있습니다. - name: Setup Ruby uses: ruby/setup-ruby@v1 with: ruby-version: '3.2' bundler-cache: true 위와 같이, Git 블로그 .github/workflows/jekyll.yml 파일에서...","categories": ["web-dev"],
        "tags": [],
        ...
        "excerpt":"MapStruct 라이브러리 추가 방법 pom.xml 수정 &lt;dependencies&gt; &lt;!-- MapStruct 라이브러리 의존성 추가 --&gt; &lt;dependency&gt; &lt;groupId&gt;org.mapstruct&lt;/groupId&gt; &lt;artifactId&gt;mapstruct&lt;/artifactId&gt; &lt;version&gt;1.4.2.Final&lt;/version&gt; &lt;/dependency&gt; &lt;/dependencies&gt; &lt;build&gt; &lt;!-- 생략 --&gt; &lt;plugins&gt; &lt;plugin&gt; &lt;groupId&gt;org.apache.maven.plugins&lt;/groupId&gt; &lt;artifactId&gt;maven-compiler-plugin&lt;/artifactId&gt; &lt;version&gt;3.10.1&lt;/version&gt; &lt;configuration&gt; &lt;source&gt;1.8&lt;/source&gt; &lt;target&gt;1.8&lt;/target&gt; &lt;encoding&gt;UTF-8&lt;/encoding&gt; &lt;!-- 항상 전체 컴파일 설정 : DTO, VO 필드 변경 시 MapStruct impl 갱신 --&gt; &lt;useIncrementalCompilation&gt;false&lt;/useIncrementalCompilation&gt; &lt;annotationProcessorPaths&gt; &lt;!--...","categories": ["web-dev"],
        "tags": [],
        "url": "/web-dev/2025-11-25-2",
        "teaser": null
      },{
        "title": "윈도우 Claude AI 설치 및 Claude CLI 사용 방법",
        "excerpt":"윈도우 CLI claude 설치 방법 윈도우 PowerShell 실행 윈도우 검색 &gt; PowerShell 입력 &gt; Windows PowerShell 실행 cmd에서는 질문할 때 여러 줄 불여넣기가 되지 않아 PowerShell로 하는 것이 좋습니다. node 설치 확인 node -v npm -v Window에서 Claude AI를 사용하기 위해서는 node가 설치되어야 합니다. claude 설치 명령어 npm install -g...","categories": ["tool"],
        "tags": [],
        ...
        "excerpt":"MSA 환경에서 여러 서비스가 공통으로 사용하는 회원 관리 플랫폼 API 프로젝트를 분석하였습니다. JWT 토큰 인증 분석 세션 대신 JWT 사용 이유 세션 인증 방식은 상태를 서버에 저장하므로 세션 저장소에 의존성이 발생하지만, JWT는 인증 정보를 토큰에 담아 전달해서 각 서비스가 독립적으로 검증하는 방식도 가능합니다. 모바일 앱 등 다양한 클라이언트 환경에서도 인증이...","categories": ["web-dev"],
        "tags": [],
        ...
        "excerpt":"부하테스트 서버 생성 이유 테스트하려는 시스템과 부하테스트 툴은 반드시 독립적으로 분리해서 구성해야 합니다. 부하테스트 툴이 컴퓨팅 리소스 (CPU, 메모리) 를 사용해서 트래픽을 만들어내기 때문입니다. Windows 또는 Mac OS 개인 컴퓨터는 부하를 발생시킬 때 제한이 걸릴 때가 종종 있으므로, 부하테스트는 EC2 인스턴스를 생성해 Linux 환경에서 진행하는 것을 권장합니다. EC2 서버에 K6...","categories": ["test"],
        "tags": [],
        ...
        "excerpt":"AWS EC2 서버 이중화 방법 (수동 확장) 느린 DB 쿼리에서 사용하는 날짜 컬럼 인덱스를 생성하여 RDS 병목 지점을 개선했었습니다. 쓰루풋이 3RPS에서 4RPS로 증가했지만, 목표 쓰루풋을 달성하기 위해 EC2 병목 지점도 개선하려고 합니다. 내부에 비효율적인 로직이 없다는 가정 하에 EC2 서버 이중화를 통해 성능 개선을 해보겠습니다. AWS 오토 스케일링 기능을 사용하지...","categories": ["test"],
        "tags": [],
