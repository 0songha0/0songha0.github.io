var store = [{
        ...
        "excerpt":"엘라스틱서치 매핑 종류 동적 매핑 logstash로 데이터 수집 시 인덱스가 없다면 기본 매핑으로 엘라스틱서치에서 인덱스를 자동 생성해주는 것입니다. 기본 매핑 종류 숫자 : long 타입 날짜 : date 타입 문자열 : text 타입 + keyword 타입 (멀티 필드) 명시적 매핑 개발자가 미리 인덱스 매핑정보를 정의해두는 것입니다. 인덱스 규모가 커질수록 동적...","categories": ["elk"],
        "tags": [],
        ...
        "excerpt":"형태소 분석기 (Analyzer) 형태소 분석기 구성요소 캐릭터 필터 : 불필요한 문자열 제거 &gt; 토크나이저 : 문자열을 토큰으로 분리 &gt; 토큰 필터 : 토큰 변경/추가/삭제 엘라스틱서치는 글로벌 오픈소스라서 영어 문법에 특화된 standard 분석기가 기본 분석기로 적용되어 있습니다. standard 분석기는 한글을 단어가 아닌 공백으로만 분리하여 저장하기 때문에 한글 분석이 안 됩니다. 한글...","categories": ["elk"],
        "tags": [],
        ...
        "excerpt":"DidimNow 사용법 디딤 문의 방법 DidimNow 로그인 &gt; 고객지원 &gt; AS 요청 &gt; 글쓰기 운영서버 장애 원인 확인 요청, WAS 서버 또는 DB 커넥션 풀 설정 확인 및 변경 요청, VM 서버 ACG 인바운드 방화벽 정책 하용 요청 등이 가능합니다. DB 커넥션 설정 값 maxTotal : WAS 서버 커넥션 풀...","categories": ["op"],
        "tags": [],
        "url": "/op/2025-01-16-1",
        "teaser": null
      },{
        ...
        "excerpt":"사내 정보보안팀에서 제공한 웹 취약점 점검 결과보고서를 통해 취약점 조치를 진행합니다. 웹 취약점 점검도구 Burp-Suite : 프록시 툴 OWASP-ZAP : 웹 애플리케이션 취약점 스캔 및 자동화 공격도구 웹 취약점 점검 항목 크로스 사이트 스크립팅 - XSS 취약점 발생 예시 공지사항 등록 시, 에디터에 아래의 크로스 사이트 스크립트 공격 구문 입력...","categories": ["op"],
        "tags": [],
        "url": "/op/2025-02-17-1",
        "teaser": null
      },{
        ...
        "excerpt":"카프카 프로젝트 분석을 위해서 학습한 내용을 정리한 글입니다. 카프카 개념 카프카란? 카프카는 중앙 메시지 허브처럼 동작하는 분산 이벤트 스트리밍 플랫폼입니다. 실시간 데이터 파이프라인, 스트리밍 애플리케이션 구축에 적합합니다. 여러 애플리케이션이 카프카를 통해 메시지 발행(Publish) 하고, 구독(Subscribe) 할 수 있습니다. 링크드인에 데이터를 생성하는 소스 애플리케이션과 데이터를 적재하는 타깃 애플리케이션이 늘어나면서 아키텍처가 복잡해짐에...","categories": ["kafka"],
        "tags": [],
