var store = [{
        ...
        "excerpt":"MSA 환경에서 여러 서비스가 사용하는 공통 회원 관리 플랫폼 API 프로젝트를 분석하였습니다. JWT 토큰 인증 분석 세션 대신 JWT 사용 이유 세션 인증 방식은 상태를 서버에 저장하므로 세션 저장소에 의존성이 발생하지만, JWT는 인증 정보를 토큰에 담아 전달해서 각 서비스가 독립적으로 검증하는 방식도 가능합니다. 다양한 클라이언트 환경 (앱, 웹뷰 등) 에서도...","categories": ["web-dev"],
        "tags": [],
